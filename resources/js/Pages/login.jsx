...
export default function Login() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        let welcomeName = ''
        if (responseData.user?.prenom && responseData.user.prenom.trim() !== '') {
          welcomeName = responseData.user.prenom
        } else if (responseData.user?.nom && responseData.user.nom.trim() !== '') {
  ...
  return (
    <>
      {showWelcomeLoader && (
        <WelcomeLoader userName={welcomeUserName} redirectUrl={redirectUrl} />
      )}

      <div className="min-h-screen flex flex-col" style={{ background: "linear-gradient(135deg, #6B46C1 0%, #1E40AF 50%, #B6C01A 100%)" }}>
        ...
        <div className="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-md text-center animate-fadeIn">
          ...
          {/* Logo institutionnel */}
          <div className="mb-6">
            <img src="/images/image.png" alt="Logo Jubilé" className="mx-auto w-24 h-24 object-contain" />
            <h1 className="text-xl font-bold text-[#1e2a78] mt-2">JUBILÉ DE COCODY</h1>
          </div>
          ...
          {step === 1 && (
            <div className="animate-slideFade">
              ...
              <p className="text-gray-600 mb-6">Entrez votre identifiant</p>
          ...
          {step === 2 && (
            <div className="animate-slideFade">
              <h2 className="text-2xl font-bold text-gray-800 mb-4">Connexion</h2>
              <p className="text-gray-600 mb-6">Bonjour, <span className="font-semibold">{data.identifiant}</span></p>
