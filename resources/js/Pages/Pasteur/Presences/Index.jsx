...
export default function PasteurPresenceDashboard({
    stats = {},
    classesData = [],
    activitesData = [],
    periodesData = [],
    ...
    const tendancesSeries = useMemo(() => {
        return [...tendancesRowsFiltres]
            .map((row, index) => {
                const weekNumber = Number.parseInt(
                    String(row?.semaine ?? "").replace(/\D+/g, ""),
                    10,
                );
    ...
    }, [tendancesRowsFiltres]);

    const tendanceLatest =
        tendancesSeries.length > 0
            ? tendancesSeries[tendancesSeries.length - 1]
            : null;
    const tendancePrevious =
        tendancesSeries.length > 1
            ? tendancesSeries[tendancesSeries.length - 2]
            : null;
    const tendanceDelta =
        tendanceLatest && tendancePrevious
            ? Math.round(tendanceLatest.pct - tendancePrevious.pct)
            : 0;
    const tendanceDirection =
        tendanceDelta > 0 ? "hausse" : tendanceDelta < 0 ? "baisse" : "stable";

    const tendanceMoyenne =
        tendancesSeries.length > 0
            ? Math.round(
                  tendancesSeries.reduce((acc, row) => acc + row.pct, 0) /
                      tendancesSeries.length,
              )
            : 0;

    const tendancePic =
        tendancesSeries.length > 0
            ? tendancesSeries.reduce((max, row) =>
                  row.pct > max.pct ? row : max,
              )
            : null;

    const tendancePlancher =
        tendancesSeries.length > 0
            ? tendancesSeries.reduce((min, row) =>
                  row.pct < min.pct ? row : min,
              )
            : null;

    const classesTopProgression = useMemo(
        () =>
            [...classesRows]
    ...
    const activitesFragiles = useMemo(() => {
        return [...activitesRowsFiltres]
            .filter((row) => Number(row?.pct ?? 0) <= 60)
            .sort((a, b) => (a.pct ?? 0) - (b.pct ?? 0))
            .slice(0, 5);
    }, [activitesRowsFiltres]);

    const tendancesAlertes = useMemo(() => {
        const items = [];

        if (tendanceDelta <= -10 && tendanceLatest) {
            items.push({
                titre: "Baisse marquée de la dynamique",
                detail: `${Math.abs(tendanceDelta)} points perdus entre ${tendancePrevious?.semaine ?? "la période précédente"} et ${tendanceLatest.semaine}.`,
                severite: "high",
            });
        }

        if (activitesFragiles.length > 0) {
            const activite = activitesFragiles[0];
            items.push({
                titre: "Activité irrégulière à surveiller",
                detail: `${activite.name} est à ${activite.pct}% de présence.`,
                severite: activite.pct <= 45 ? "high" : "medium",
            });
        }

        if (alertesRowsFiltres.length > 0) {
            const topAbsence = [...alertesRowsFiltres].sort(
                (a, b) => (b.absences ?? 0) - (a.absences ?? 0),
            )[0];
            items.push({
                titre: "Absences récurrentes en hausse",
                detail: `${topAbsence.name} (${topAbsence.classe}) cumule ${topAbsence.absences} absences.`,
                severite: (topAbsence.absences ?? 0) >= 5 ? "high" : "medium",
            });
        }
    ...
    const tendanceActions = useMemo(() => {
        return tendancesAlertes.map((alerte, index) => ({
            priorite: alerte.severite === "high" ? "Haute" : "Moyenne",
            axe: alerte.titre.includes("Activité")
                ? "Activités"
                : alerte.titre.includes("Absences")
                  ? "Membres"
                  : "Suivi global",
            action:
                alerte.severite === "high"
                    ? "Planifier un suivi ciblé cette semaine"
                    : "Surveiller et envoyer un rappel préventif",
            id: `${alerte.titre}-${index}`,
        }));
    }, [tendancesAlertes]);
    ...
    return (
        <div style={styles.dash}>
            ...
            {/* Stats mois + Top 3 activités */}
            {(() => {
                const now = new Date();
                const moisCourant = now.toLocaleDateString("fr-FR", { month: "long" }).toLowerCase();
                const moisLabel = now.toLocaleDateString("fr-FR", { month: "long", year: "numeric" });
                const statsMois = periodesData.find((p) => (p.mois ?? "").toLowerCase() === moisCourant);
                ...
                return (
                    <div style={{ padding: "16px 24px 0", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
                        <div style={{ background: "rgba(255,255,255,0.08)", borderRadius: 14, padding: "16px 20px", border: "1px solid rgba(255,255,255,0.12)" }}>
                            <p style={{ margin: "0 0 14px", fontWeight: 700, fontSize: 13, color: "rgba(255,255,255,0.7)", textTransform: "uppercase", letterSpacing: 1 }}>📅 {moisLabel}</p>
                            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                                <div style={{ background: "rgba(67,160,71,0.15)", borderRadius: 10, padding: "12px 14px", border: "1px solid rgba(67,160,71,0.3)" }}>
                                    <div style={{ fontSize: 26, fontWeight: 800, color: "#81c784" }}>{nbPresentsMois}<span style={{ fontSize: 13, fontWeight: 500, marginLeft: 6, color: "#a5d6a7" }}>· {tauxMois}%</span></div>
                                    ...
                                    <div style={{ height: 4, background: "rgba(255,255,255,0.1)", borderRadius: 10, marginTop: 8, overflow: "hidden" }}>
                                        <div style={{ height: "100%", width: `${tauxMois}%`, background: "#43a047", borderRadius: 10 }} />
                                    </div>
                                </div>
                                <div style={{ background: "rgba(229,57,53,0.15)", borderRadius: 10, padding: "12px 14px", border: "1px solid rgba(229,57,53,0.3)" }}>
                                    <div style={{ fontSize: 26, fontWeight: 800, color: "#ef9a9a" }}>{nbAbsentsMois}<span style={{ fontSize: 13, fontWeight: 500, marginLeft: 6, color: "#ffcdd2" }}>· {nbTotalMois > 0 ? Math.round((nbAbsentsMois / nbTotalMois) * 100) : 0}%</span></div>
                                    <div style={{ fontSize: 11, color: "rgba(255,255,255,0.6)", marginTop: 3 }}>Absences ce mois</div>
                                    <div style={{ height: 4, background: "rgba(255,255,255,0.1)", borderRadius: 10, marginTop: 8, overflow: "hidden" }}>
                                        <div style={{ height: "100%", width: `${nbTotalMois > 0 ? Math.round((nbAbsentsMois / nbTotalMois) * 100) : 0}%`, background: "#e53935", borderRadius: 10 }} />
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div style={{ background: "rgba(255,255,255,0.08)", borderRadius: 14, padding: "16px 20px", border: "1px solid rgba(255,255,255,0.12)" }}>
                            <p style={{ margin: "0 0 14px", fontWeight: 700, fontSize: 13, color: "rgba(255,255,255,0.7)", textTransform: "uppercase", letterSpacing: 1 }}>🏆 Top activités</p>
                            {top3.length === 0 ? (
                                    ...
                                    {top3.map((a, i) => (
                                        <div key={i}>
                                            ...
                                            <div style={{ display: "flex", gap: 6, marginBottom: 6 }}>
                                                <span style={{ fontSize: 11, padding: "2px 8px", borderRadius: 8, background: "rgba(67,160,71,0.25)", color: "#a5d6a7", fontWeight: 600 }}>✔ {a.presents} présents</span>
                                                <span style={{ fontSize: 11, padding: "2px 8px", borderRadius: 8, background: "rgba(229,57,53,0.25)", color: "#ef9a9a", fontWeight: 600 }}>✗ {a.absents} absents</span>
                                            </div>
                                            <div style={{ height: 5, background: "rgba(255,255,255,0.1)", borderRadius: 10, overflow: "hidden" }}>
                                                <div style={{ height: "100%", width: `${Math.round((a.presents / maxPresents) * 100)}%`, background: barColors[i], borderRadius: 10 }} />
                                            </div>
                                        </div>
                                    ))}
                                </div>
                                      ...
                                      0 ? (
                                        ...
                                        <div style={{ overflowX: "auto" }}>
                                            ...
                                            >
                                                ...
                                                <tbody>
                                                    {participantsActivitePagines.map((row, index) => {
                                                                ...
                                                                <td style={tdStyle}>
                                                                    <button
                                                                        onClick={() => {
                                                                            ...
                                                                            setModalAbsencesParticipant({ nom: row.participant, classe: row.classe, absences });
                                                                        }}
                            ...
                            <>
                                ...
                                {totalPages > 1 && (
                                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 16, paddingTop: 12, borderTop: "1px solid #e5e7eb" }}>
                                        <span style={{ fontSize: 12, color: "#888" }}>Page {pageSafe}/{totalPages}</span>
                                        <div style={{ display: "flex", gap: 6 }}>
                                            ...
                                            {Array.from({ length: Math.min(totalPages, 7) }, (_, i) => {
                                                ...
                                                return (
                                                    <button key={page} onClick={() => setParticipantsPage(page)} style={{ width: 30, height: 30, borderRadius: 8, border: "1px solid #d9dcf2", background: page === pageSafe ? "#1e2070" : "#fff", color: page === pageSafe ? "#fff" : "#1e2070", fontSize: 12, fontWeight: 600, cursor: "pointer" }}>
                                                        {page}
                                                    </button>
                                                );
                                            })}
                    ...
                    >
                        ...
                        <div style={{ overflowY: "auto", flex: 1, display: "flex", flexDirection: "column", gap: 8 }}>
                            {modalAbsencesParticipant.absences.length === 0 ? (
                                <p style={{ color: "#888", textAlign: "center", padding: "20px 0", fontSize: 13 }}>Aucune absence enregistrée.</p>
                            ) : (
                                modalAbsencesParticipant.absences.map((a, i) => (
                                    <div key={i} style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 12px", background: "#fff5f5", borderRadius: 10, border: "1px solid #fecaca" }}>
                                        <div style={{ flex: 1 }}>
                                            <p style={{ margin: 0, fontSize: 13, fontWeight: 600, color: "#222" }}>{a.activite || "—"}</p>
                                            <p style={{ margin: 0, fontSize: 11, color: "#888" }}>{formatParticipantDate(a.date)}</p>
                                        </div>
                                        <span style={{ fontSize: 11, padding: "3px 10px", borderRadius: 8, background: "#fee2e2", color: "#991b1b", fontWeight: 700 }}>Absent</span>
                                    </div>
                                ))
                            )}
                        </div>
