...
export default function PasteurPrieresIndex({ prayerRequests = [] }) {
    const [requests, setRequests] = useState(() => normalizeRequests(prayerRequests));
    ...
    const stats = useMemo(() => {
        ...
        const inPrayer = requests.filter((r) => r.status === "En priere").length;
        const fulfilledCount = requests.filter((r) => r.status === "Exaucement" || r.status === "Exaucement partage").length;
        const anonymousCount = requests.filter((r) => r.isAnonymous).length;

        return [
            { title: "Demandes recues", value: total, subtitle: "Toutes les demandes de priere recues depuis les differents espaces.", icon: MessageSquareHeart, iconClass: "bg-sky-100 text-sky-700" },
            { title: "En priere", value: inPrayer, subtitle: "Demandes actuellement suivies dans la priere pastorale.", icon: HeartHandshake, iconClass: "bg-blue-100 text-blue-700" },
            { title: "Mode anonyme", value: anonymousCount, subtitle: "Demandes preservees sans affichage du nom et prenom.", icon: EyeOff, iconClass: "bg-violet-100 text-violet-700" },
            { title: "Exaucements", value: fulfilledCount, subtitle: "Demandes explicitement marquees comme exaucees.", icon: CheckCheck, iconClass: "bg-emerald-100 text-emerald-700" },
        ];
    }, [requests]);
    ...
    const toggleReaction = (requestId, commentId, emoji) => {
        router.patch(`/pasteur/prieres/${requestId}/reaction`, { comment_id: commentId, emoji }, { preserveScroll: true });
        setShowReactionPicker(null);
    };
    ...
    const submitComment = (requestId) => {
        ...
        router.patch(`/pasteur/prieres/${requestId}/commentaire`, { commentaire: draft, reply_to_comment_id: replyTargets[requestId] ?? null }, {
            preserveScroll: true,
            onSuccess: () => {
                setCommentDrafts((current) => ({ ...current, [requestId]: "" }));
                setReplyTargets((current) => ({ ...current, [requestId]: null }));
            },
        });
    };
    ...
    const activeTabContent = useMemo(() => {
        const contents = {
            new: { title: "Nouvelles demandes de priere", description: "Retrouvez ici les nouvelles demandes recues qui attendent encore une premiere prise en charge." },
            in_prayer: { title: "Demandes en priere", description: "Retrouvez ici les demandes actuellement portees dans la priere pastorale." },
            fulfilled: { title: "Demandes exaucees", description: "Retrouvez ici les demandes explicitement marquees comme exaucees." },
            all: { title: "Toutes les demandes de priere", description: "Retrouvez ici les demandes recues et mettez a jour leur prise en charge." },
        };
    ...
    return (
        <>
            ...
            <div className="min-h-screen px-4 py-8 sm:px-6 lg:px-10" style={{ background: "linear-gradient(135deg, #6B46C1 0%, #1E40AF 50%, #B6C01A 100%)" }}>
                ...
                <div className="relative w-full">
                    ...
                    <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                        {stats.map((stat) => (
                            <div key={stat.title} className="flex h-full flex-col rounded-[24px] border border-white/70 bg-white/95 p-5 shadow-[0_20px_50px_rgba(15,23,42,0.08)]">
                                <div className="flex items-start justify-between gap-4">
                                    <p className="text-xs font-semibold uppercase tracking-[0.22em] text-slate-500">{stat.title}</p>
                                    <span className={`inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-xl ${stat.iconClass}`}>
                                        <stat.icon className="h-5 w-5" />
                                    </span>
                                </div>
                                <p className="mt-3 text-3xl font-bold tracking-tight text-slate-900">{stat.value}</p>
                                <p className="mt-2 text-sm leading-6 text-slate-600">{stat.subtitle}</p>
                            </div>
                        ))}
                    </div>
                    ...
                    <section className="mt-8 overflow-hidden rounded-[34px] border border-white/70 bg-[linear-gradient(180deg,rgba(255,255,255,0.98)_0%,rgba(248,250,252,0.97)_100%)] shadow-[0_24px_70px_rgba(15,23,42,0.14)]">
                        <div className="border-b border-slate-200/80 bg-[linear-gradient(180deg,rgba(255,255,255,0.98),rgba(248,250,252,0.96))] px-4 py-4">
                            <div className="flex flex-wrap items-center justify-between gap-4">
                                <div>
                                    <h2 className="text-xl font-bold tracking-tight text-slate-900">{activeTabContent.title}</h2>
                                    <p className="mt-1.5 max-w-2xl text-sm leading-5 text-slate-600">{activeTabContent.description}</p>
                                </div>
                                <div className="flex flex-wrap gap-1.5">
                                    {tabs.map((tab) => {
                                        const isActive = activeTab === tab.id;
                                        return (
                                            <button key={tab.id} type="button" onClick={() => setActiveTab(tab.id)} className={`inline-flex items-center gap-2 rounded-xl border px-3 py-1.5 text-[13px] font-semibold transition ${isActive ? "border-blue-500 bg-blue-600 text-white shadow-[0_12px_28px_rgba(37,99,235,0.30)]" : "border-slate-200 bg-white text-slate-700 hover:border-slate-300 hover:bg-slate-50"}`}>
                                                <span>{tab.label}</span>
                                                <span className={`rounded-full px-2 py-0.5 text-xs ${isActive ? "bg-white/20 text-white" : "bg-slate-100 text-slate-600"}`}>{tab.count}</span>
                                            </button>
                                        );
                                    })}
                                </div>
                        ...
                        <div className="px-4 py-4">
                            {filteredRequests.length === 0 ? (
                                <div className="rounded-[30px] border border-dashed border-slate-300 bg-[linear-gradient(180deg,#f8fafc_0%,#f1f5f9_100%)] px-6 py-14 text-center">
                                    <p className="text-base font-semibold text-slate-900">Aucune demande dans cet onglet.</p>
                                    <p className="mt-2 text-sm leading-6 text-slate-600">Ajustez le filtre ou attendez de nouvelles demandes.</p>
                                </div>
                            ) : (
                                <div className="grid gap-4">
                                    {filteredRequests.map((request) => {
                                        const lastComment = request.comments?.length > 0 ? request.comments[request.comments.length - 1] : null;
                                                    ...
                                                    <div className="min-w-0">
                                                        <div className="p-5 xl:p-6">
                                                            <div className="flex flex-wrap items-center gap-2">
                                                                <span className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[11px] font-semibold ${statusConfig.badge}`}>
                                                                ...
                                                                <span className="inline-flex items-center gap-1.5 rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-700 ring-1 ring-slate-200">
                                                                    {request.isAnonymous ? <ShieldCheck className="h-3 w-3" /> : <Eye className="h-3 w-3" />}
                                                                    {request.authorLabel}
                                                                </span>
                                                                {request.sourceLabel && (
                                                                ...
                                                                {request.createdAt && <span className="ml-auto text-[11px] font-medium text-slate-500">{request.createdAt}</span>}
                                                            </div>

                                                            <h3 className="mt-4 text-base font-bold tracking-tight text-slate-900 sm:text-[1.02rem]">{request.subject}</h3>

                                                            <div className="mt-3">
                                                                {isExpanded ? (
                                                                    <div className="rounded-2xl border border-slate-200 bg-slate-50/80 p-4">
                                                                        <p className="whitespace-pre-wrap text-sm leading-7 text-slate-700">{request.message}</p>
                                                                        <button
                                                                            type="button"
                                                                    ...
                                                                    <div className="flex flex-wrap items-center gap-2">
                                                                        <span className="text-sm italic text-slate-500">{getMessagePreview(request.message)}</span>
                                                                        <button
                                                                            type="button"
                                                                            onClick={() => handleToggleDetails(request)}
                                                        ...
                                                        <div>
                                                            <div>
                                                                <div className="flex items-start justify-between gap-3">
                                                                    <div>
                                                                        <p className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">Echange pastoral</p>
                                                                        <p className="mt-2 text-sm font-bold text-slate-900">
                                                                            {commentsCount > 0 ? `${commentsCount} message${commentsCount > 1 ? "s" : ""}` : "Aucun echange"}
                                                                        </p>
                                                                    </div>
                                                                </div>
                                                                ...
                                                                <div className="mt-3 rounded-2xl border border-slate-200 bg-white p-3 shadow-sm">
                                                                    {lastComment ? (
                                                                        <div className="space-y-2">
                                                                            <p className="text-xs font-semibold text-slate-600">{lastComment.actorLabel}</p>
                                                                            <p className="mt-1 line-clamp-2 text-sm leading-5 text-slate-600">{lastComment.message}</p>
                                                                        </div>
                                                                    ) : (
                                                                        <p className="text-sm leading-5 text-slate-500">Aucun echange pour l&apos;instant. Ouvrez le fil pour demarrer l&apos;accompagnement.</p>
                                                                    )}
                                                                </div>
                                                            </div>
                                                        </div>
                    ...
                    <div className="relative flex w-full max-w-[600px] flex-col overflow-hidden rounded-l-[28px] border-l border-white/80 bg-white shadow-[0_24px_70px_rgba(15,23,42,0.18)]">
                            ...
                            <div className="flex items-center justify-between gap-4">
                                <div className="min-w-0 flex-1">
                                    ...
                                    <div className="mt-1 flex items-center gap-2">
                                        ...
                                        <span className="text-[11px] text-slate-500">{currentThreadData.authorLabel}</span>
                                    </div>
                                </div>
                                <button onClick={() => setOpenThread(null)} className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl border border-slate-200 bg-white text-slate-500 transition hover:bg-slate-100 hover:text-slate-700">
                                    <X className="h-4 w-4" />
                                </button>
                            </div>
                        </div>
                                        ...
                                        <div key={`${openThread}-c-${comment.id ?? index}`} className={`flex ${isPastor ? 'justify-end' : 'justify-start'}`}>
                                            <div className={`group relative max-w-[85%] ${isPastor ? 'order-2' : 'order-1'}`}>
                                                ...
                                                <div className={`mt-1.5 flex items-center gap-3 px-1 ${isPastor ? 'justify-end' : 'justify-start'}`}>
                                                    ...
                                                    {currentThreadData.canComment && (
                                                        <div className="flex items-center gap-1 opacity-0 transition group-hover:opacity-100">
                                                            <button onClick={() => setReplyTargets(current => ({...current, [openThread]: comment.id}))} className="rounded-full p-1 text-slate-400 transition hover:bg-slate-200/80 hover:text-slate-700">
                                                                <CornerDownRight className="h-3 w-3" />
                                                            </button>
                                                            <button onClick={() => setShowReactionPicker(showReactionPicker === comment.id ? null : comment.id)} className="rounded-full p-1 text-slate-400 transition hover:bg-slate-200/80 hover:text-slate-700">
                                                ...
                                                {comment.reactions?.length > 0 && (
                                                    <div className={`mt-1 flex flex-wrap gap-1 ${isPastor ? 'justify-end' : 'justify-start'}`}>
                                                        {comment.reactions.map(r => (
                                                            <span key={r.emoji} className="inline-flex items-center gap-1 rounded-full border border-slate-200 bg-white/95 px-2.5 py-1 text-[11px] text-slate-700 shadow-sm">
                                                                {r.emoji} {r.count}
                                                            </span>
                                                        ))}
                                                    </div>
                                                ...
                                                {/* Picker de réactions */}
                                                {showReactionPicker === comment.id && comment.canReact && (
                                                    <div className={`absolute top-0 z-10 mt-[-52px] flex gap-1 rounded-2xl border border-slate-200 bg-[linear-gradient(180deg,#ffffff_0%,#f8fafc_100%)] p-2.5 shadow-[0_18px_40px_rgba(15,23,42,0.14)] ${isPastor ? 'right-0' : 'left-0'}`}>
                                                        {AVAILABLE_REACTIONS.map(emoji => (
                                                            <button
                                                                key={emoji}
                                                                onClick={() => toggleReaction(openThread, comment.id, emoji)}
                                                                disabled={comment.hasPastorReaction}
                        ...
                        {currentThreadData.canComment ? (
                            <div className="border-t border-slate-200 bg-[linear-gradient(180deg,#ffffff_0%,#f8fafc_100%)] px-5 py-4">
                                    ...
                                    <div className="mb-3 flex items-center justify-between rounded-xl border border-slate-200 bg-slate-50 px-4 py-2.5">
                                        <div className="min-w-0 flex-1">
                                            ...
                                            <p className="truncate text-[11px] text-slate-500">
                                                {formatReplySnippet(currentThreadData.comments.find(c => c.id === replyTargets[openThread])?.message)}
                                            </p>
                                        </div>
                                        <button onClick={() => setReplyTargets(current => ({...current, [openThread]: null}))} className="ml-2 rounded-full p-1 text-slate-400 transition hover:text-slate-700">
                                ...
                                <div className="flex items-end gap-3">
                                    ...
                                    <button
                                        ...
                                        disabled={!commentDrafts[openThread]?.trim()}
                            ...
                            <div className="border-t border-slate-200 bg-slate-50 px-6 py-4 text-center text-[12px] text-slate-500">
                                {currentThreadData.status === "Exaucement" || currentThreadData.status === "Exaucement partage"
