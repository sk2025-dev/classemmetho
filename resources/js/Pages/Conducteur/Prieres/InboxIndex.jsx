...
function PrayerList({ title, description, requests, commentDrafts, setCommentDrafts, routeBase, readOnly = false }) {
    return (
                    ...
                    {requests.map((request) => (
                        <article key={request.id} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                            <div className="flex flex-wrap items-center gap-2 text-xs text-slate-500">
                                <span className="rounded-full bg-slate-100 px-3 py-1 font-semibold text-slate-700">{request.status}</span>
                                <span>{request.createdAt}</span>
                                {request.targetLabel ? <span>• {request.targetLabel}</span> : null}
                            </div>
                            <h3 className="mt-3 text-base font-bold text-slate-900">{request.subject}</h3>
                            <p className="mt-2 text-sm leading-6 text-slate-600">{request.message}</p>
                            {request.sourceLabel ? <p className="mt-2 text-xs font-medium text-blue-700">Source: {request.sourceLabel}</p> : null}
                            {request.testimony ? (
                            ...
                            {!readOnly ? (
                                request.comments?.length > 0 ? (
                                    <button
                                        type="button"
                                        onClick={() => router.patch(`${routeBase}/${request.id}/exaucee`, {}, { preserveScroll: true })}
                                        ...
                                        <button
                                            ...
                                            onClick={() =>
                                                router.patch(
                                                    ...
                                                    { temoignage: commentDrafts[request.id] || "" },
                                                    { preserveScroll: true },
                                                )
                                            }
...
export default function ConducteurPrieresInboxIndex({
    ...
    useEffect(() => setInboxRequests(receivedPrayerRequests), [receivedPrayerRequests]);

    return (
        <>
            <Head title="Prieres - Conducteur" />
            <div className="min-h-screen px-4 py-6 sm:px-6 lg:px-8" style={{ background: "linear-gradient(135deg, #6B46C1 0%, #1E40AF 50%, #B6C01A 100%)" }}>
                <div className="w-full">
                    <div className="flex items-center gap-3 text-white">
                        <Link href="/conducteur/dashboard" className="inline-flex h-10 w-10 items-center justify-center rounded-full bg-white/10 transition hover:bg-white/20">
                            <ArrowLeft className="h-5 w-5" />
                        </Link>
                        <div>
                            <h1 className="text-3xl font-bold tracking-tight sm:text-4xl">Demandes de priere</h1>
                            <p className="text-sm text-blue-100">Vos demandes personnelles et les prières reçues de votre classe.</p>
                        </div>
                    </div>
                    ...
                    <div className="mt-6 grid gap-4 lg:grid-cols-[360px_minmax(0,1fr)]">
                        <section className="rounded-[28px] border border-white/70 bg-white/95 p-4 shadow-[0_24px_70px_rgba(15,23,42,0.14)]">
                            ...
                            <p className="mt-1 text-sm text-slate-600">
                                {authUser?.prenom} {authUser?.nom}, vous pouvez toujours envoyer votre propre demande.
                            </p>
                            <form
                                onSubmit={(event) => {
                                    event.preventDefault();
                                    router.post("/conducteur/prieres", {
                                        sujet: subject,
                                        demande: message,
                            ...
                            >
                                <input value={subject} onChange={(event) => setSubject(event.target.value)} placeholder="Sujet" className="w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm outline-none focus:border-blue-400 focus:ring-4 focus:ring-blue-100" />
                                <textarea value={message} onChange={(event) => setMessage(event.target.value)} rows={5} placeholder="Votre demande" className="w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm outline-none focus:border-blue-400 focus:ring-4 focus:ring-blue-100" />
                                <button type="submit" className="inline-flex items-center gap-2 rounded-2xl bg-teal-600 px-4 py-3 text-sm font-semibold text-white hover:bg-teal-700">
