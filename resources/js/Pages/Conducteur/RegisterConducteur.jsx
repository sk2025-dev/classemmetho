...
const STYLES = {
  button: {
    ...
    danger:
      "px-3 py-2 text-red-600 hover:bg-red-50 hover:text-red-700 rounded-lg transition-all duration-300 flex items-center gap-2 font-medium",
    small:
      "px-4 py-2 rounded-lg bg-blue-600 text-white font-medium shadow hover:shadow-md hover:bg-blue-700 transition-all duration-300",
  },
  input:
...
export default function RegisterConducteur({
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const classesRes = await fetch(withBasePath("", "/api/classes"));
        if (classesRes.ok) {
        ...
        const rolesRes = await fetch(withBasePath("", "/api/fonctions"));
        if (rolesRes.ok) {
  ...
  }, [churchRoles, setSelectedRolesResponsable, setSelectedMembresRoles]);
  ...
  const startEditingMember = (index) => {
    ...
    const rawRoles =
      typeof member.fonction === "string"
        ? member.fonction
            .split(",")
            .map((value) => value.trim())
            .filter(Boolean)
        : Array.isArray(member.fonction)
          ? member.fonction
          : [];
    setSelectedMembresRoles(new Set(rawRoles));
    setErrors({});
  };
  ...
  const validateStep = (s) => {
    ...
    if (s === 1) {
      ...
      if (!famille.classe_id)
        newErrors["famille.classe_id"] = "Veuillez sélectionner une classe.";
    }
    if (s === 2) {
  ...
  const validateField = (fieldName, value) => {
    ...
    if (fieldName.startsWith("famille.")) {
      ...
      if (field === "nom") {
        ...
        if (!value) error = "Requis";
        else if (!isValidPhoneFormat(value)) error = "10 chiffres exact";
  ...
  const handleSubmit = async (e) => {
    ...
    Object.entries(responsable).forEach(([k, v]) => {
      ...
      if (k === "photo") {
        if (responsablePhotoToSend) {
          formData.append(`responsable[photo]`, responsablePhotoToSend);
        }
        return;
      }
    ...
    formData.append("employment_status", responsable.employment_status || "");
    formData.append("profession_detail", responsable.profession_detail || "");
    ...
    const spirituel = {
      ...
      marieReligieusement: responsable.marieReligieusement ? "true" : "false",
    ...
    if (membres.length > 0) {
      membres.forEach((m, i) => {
        const memberPhotoToSend = normalizePhotoValue(m.photo, m.photoPreview);
        Object.entries(m).forEach(([k, v]) => {
          if (k === "photo") {
            if (memberPhotoToSend) {
              formData.append(`membres[${i}][photo]`, memberPhotoToSend);
            }
      ...
      case 1:
        return (
          <div className="space-y-6 animate-fadeIn">
            <FormField label="Nom de la famille" icon={Home} required>
              <input
                ref={familyNameRef}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <FormField label="Ville" icon={Building} required>
                <Select2Single
                  ...
                  hasError={Boolean(getFieldError("famille.ville"))}
                />
            ...
            <FormField label="Classe de la famille" icon={BookOpen} required>
              <Select2Single
                id="classe_id_select"
                name="classe_id"
                ...
                hasError={Boolean(getFieldError("famille.classe_id"))}
              />
            ...
            <div className="w-full p-6 bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl border-2 border-blue-200 shadow-md">
              <div className="flex flex-col items-center gap-3">
                ...
                <PhotoUploadInput
                  ...
                  onUploadStateChange={setIsResponsablePhotoUploading}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <FormField label="Genre" icon={Users} required>
                <Select2Single
                  ...
                  placeholder="Sélectionner..."
                  hasError={Boolean(getFieldError("responsable.genre"))}
                />
              </FormField>
              <FormField
              ...
              >
                <Select2Single
                  ...
                  hasError={Boolean(getFieldError("responsable.lienParente"))}
                />
              </FormField>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-1 gap-4">
              <FormField label="Statut Marital" icon={Heart} required>
                <Select2Single
                  ...
                  options={RESPONSABLE_MARITAL_STATUS_OPTIONS}
                  ...
                  placeholder="Sélectionner..."
                  hasError={Boolean(getFieldError("responsable.statutMarital"))}
                />
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setResponsable({
                        ...
                        datePremiereCommunion: e.target.checked
                          ? responsable.datePremiereCommunion
                          : "",
                        lieuPremiereCommunion: e.target.checked
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setResponsable({
                        ...
                        dateMariageReligieux: e.target.checked
                          ? responsable.dateMariageReligieux
                          : "",
                        lieuMariageReligieux: e.target.checked
          ...
          <div className="space-y-6 animate-fadeIn">
            ...
            {hasMembersToAdd === null && (
              <div className="text-center space-y-6">
                ...
                <p className="text-gray-600">
                  Vous pouvez ajouter des enfants, conjoints, parents ou autres
            ...
            {hasMembersToAdd === false && (
              <div className="text-center space-y-6 bg-blue-50 p-8 rounded-lg border border-blue-200">
                ...
                <h3 className="text-2xl font-bold text-gray-800">D'accord !</h3>
                <p className="text-gray-600">
                  Vous pouvez continuer avec votre inscription sans ajouter de
                  membres pour le moment.
                </p>
                <button
                  type="button"
                  ...
                  <div className="w-full p-6 bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl border-2 border-blue-200 shadow-md">
                    <div className="flex flex-col items-center gap-3">
                      ...
                      <PhotoUploadInput
                        ...
                        onUploadStateChange={setIsMemberPhotoUploading}
                        onPhotoSelected={(photoUrl) => {
                          setMembreTemp((prev) => ({
                            ...prev,
                            photo: photoUrl,
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <FormField label="Genre" icon={Users} required>
                      <Select2Single
                        ...
                        placeholder="Sélectionner..."
                        hasError={Boolean(getFieldError("membre.genre"))}
                      />
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    >
                      <Select2Single
                        ...
                        placeholder="Sélectionner un statut"
                        hasError={Boolean(
                          getFieldError("membre.employment_status"),
                        )}
                      />
                      {getFieldError("membre.employment_status") && (
                        <p className="text-red-500 text-xs mt-1">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    >
                      <Select2Single
                        ...
                        placeholder="Sélectionner un lien de parenté"
                        hasError={Boolean(getFieldError("membre.relation"))}
                      />
                      {getFieldError("membre.relation") && (
                        <p className="text-red-500 text-xs mt-1">
                          {getFieldError("membre.relation")}
                        </p>
                      )}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 p-4 bg-white rounded-lg border border-gray-200">
                      <FormField label="Date du dot" icon={Calendar} required>
                        <input
                          type="date"
                          className={STYLES.input}
                          value={membreTemp.dateDot || ""}
                          onChange={(e) =>
                            setMembreTemp({
                              ...membreTemp,
                              dateDot: e.target.value,
                            })
                          }
                        />
                        ...
                        >
                          <div className="flex items-start justify-between gap-4">
                            ...
                            <div className="flex items-center gap-3">
                              <button
                                type="button"
                                onClick={() => startEditingMember(idx)}
          ...
          <div className="space-y-6 animate-fadeIn">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="p-4 bg-blue-50 rounded-lg border border-blue-100">
                ...
                <ul className="text-sm space-y-1 text-gray-700">
                  ...
                  <li>
                    <strong>Ville:</strong>{" "}
                    {villeSelected ? villeSelected.nom : famille.ville}
                  </li>
                  <li>
                    <strong>Quartier:</strong> {famille.quartier}
                  </li>
                  <li>
                    <strong>Adresse:</strong> {famille.adresse}
                  </li>
                  <li>
                    <strong>Tel:</strong> {famille.telephone}
                  </li>
                  <li>
                    <strong>Classe:</strong>{" "}
                    {classeSelected ? classeSelected.nom : "Non sélectionnée"}
                  </li>
                </ul>
              </div>
              <div className="p-4 bg-green-50 rounded-lg border border-green-100">
                ...
                <ul className="text-sm space-y-1 text-gray-700">
                  <li>
                    <strong>Nom:</strong> {responsable.prenom} {responsable.nom}
                  </li>
                  <li>
            ...
            <div className="p-4 bg-yellow-50 rounded-lg border border-yellow-100">
              <h4 className="font-semibold text-yellow-800 mb-2 flex items-center gap-2">
                <Users className="w-4 h-4" /> Membres de la famille (
                {membres.length})
              </h4>
              {membres.length === 0 ? (
                ...
                <ul className="space-y-2">
                  {membres.map((m, idx) => (
                    <li key={idx} className="flex items-center gap-3">
                      ...
                      {m.baptise && (
                        <span className="text-xs text-blue-600">Baptisé</span>
                      )}
                    </li>
                  ))}
                </ul>
            ...
            <div className="border-t pt-4">
              <label className="flex items-start gap-3 cursor-pointer">
                ...
                <span className="text-sm text-gray-600">
                  J'atteste l'exactitude de ces informations et accepte la
                  politique de confidentialité de l'église.
                </span>
              </label>
            </div>
          </div>
