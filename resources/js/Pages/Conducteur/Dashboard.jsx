...
export default function Dashboard({
    ...
    const menuItems = [
        ...
        ...(auth?.user?.classe?.has_tribus
            ? [
                  {
                      title: "Tribu",
                      desc: "Gérer les tribus de la classe",
                      icon: "tribu",
                      href: "/conducteur/tribus",
                    ...
                    {menuItems.map((item, index) => (
                        ...
                        >
                            ...
                            <div className="bg-white text-gray-800 rounded-xl shadow-xl hover:shadow-2xl hover:scale-105 transition-all duration-300 p-6 h-full border-2 border-yellow-400 group-hover:border-t-4 group-hover:border-blue-500 relative overflow-hidden z-10">
                                ...
                                {/* Badges inscription + transferts */}
                                {item.icon === "inscription" &&
                                    (pendingInscriptions > 0 || pendingTransfers > 0) && (
                                        <div className="absolute top-3 right-3 flex flex-col items-end gap-1">
                                            {pendingInscriptions > 0 && (
                                                <span className="inline-flex items-center gap-1 bg-red-500 text-white text-[10px] font-bold px-2 py-0.5 rounded-full shadow">
                                                    <span>✉</span>
                                                    {pendingInscriptions > 99 ? "99+" : pendingInscriptions}
                                                </span>
                                            )}
                                            {pendingTransfers > 0 && (
                                                <span className="inline-flex items-center gap-1 bg-red-500 text-white text-[10px] font-bold px-2 py-0.5 rounded-full shadow animate-pulse">
                                                    <span>⇄</span>
