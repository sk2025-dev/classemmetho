...
export default function Inscriptions({
    ...
    const inscriptionsOnly = (inscriptions || []).map((insc) => {
        const mapped = {
            ...
            // ⚠️ CORRECTION: Mapper les statuts d'inscription CORRECTEMENT
            // en_attente -> pending
            ...
            profile_photo_url: insc.profile_photo_url || null,
    ...
    // ✅ Pour l'onglet membres: afficher UNIQUEMENT les members
    const filteredItems = (activeTab === "members" ? membersOnly : inscriptionsOnly).filter((item) => {
        ...
        if (activeTab === "members") {
            ...
            if (filters.familyFilter !== "") {
                if (filters.familyFilter === "no_family") {
                    matchesFamily = !item.famille_id;
                } else {
                    // Trouver la famille du membre
                    const memberFamily = familiesList.find(f =>
                        f.members && f.members.some(m => m.id === item.id)
                    );
                    matchesFamily = memberFamily && memberFamily.familyId == filters.familyFilter;
                }
            }
    ...
    const openDetailModal = (item) => {
        setSelectedInscription(item.raw);
    ...
    const openEditModal = (item) => {
        ...
        setData({
            first_name: item.first_name || item.prenom,
            last_name: item.last_name || item.nom,
            email: item.email || "",
            phone: item.phone || item.telephone || "",
            role: item.role === "inscription" ? "membre" : item.role,
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        if (modalMode === "create") {
            // Récupérer les données du conducteur (user connecté)
            ...
            // Ajouter tous les champs du formulaire directement
            // (pas de structure imbriquée car on ajoute juste un simple membre)
            ...
            // Envoyer les données au nouvel endpoint simplifié
            ...
            const url =
                selectedMember.kind === "inscription"
                    ? `/conducteur/inscriptions/${selectedMember.id}`
                    : `/conducteur/members/${selectedMember.id}`;
            put(url, {
    ...
    const validateMember = (id) => {
        if (confirm("Confirmer l'inscription de ce membre dans la classe ?")) {
            router.put(
    ...
    const rejectMember = (id) => {
        const reason = prompt(
            "Veuillez indiquer le motif du refus (optionnel) :",
        );
        router.put(
            ...
            >
                <div className="w-full mx-auto py-8 px-2">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-5 gap-6 mb-8">
                        ...
                        <div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl p-6 text-white flex items-center gap-4">
                            <div className="w-12 h-12 rounded-full bg-blue-500/20 flex items-center justify-center text-white text-xl border border-blue-500/50">
                                <UserCheck />
                            </div>
                            <div>
                                <div className="text-sm opacity-70 uppercase tracking-wider font-medium">
                    ...
                    {activeTab === "families" ? (
                        ...
                        <div className="bg-white rounded-2xl shadow-xl overflow-hidden border border-slate-100">
                            ...
                            <div className="p-4 border-t border-slate-200 bg-slate-50 flex justify-between items-center text-sm text-slate-500">
                                <span>
                                    Affichage de {filteredItems.length}{" "}
                                    résultat(s)
                                </span>
                                <span>
                                    {activeTab === "pending" && "En attente d'approbation"}
                                    {activeTab === "approved" && "Inscriptions approuvées"}
                                                ...
                                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                    ...
                                                    <FormField label="Fonction dans l'église" icon={Users} required>
                                                        <Select2Fonction
                                                            ...
                                                            onChange={(e) => setData({...data, fonction_id: e.target.value && e.target.value.length > 0 ? e.target.value[0] : ""})}
                                                            options={fonctions}
                                                ...
                                                {selectedInscription.admin_approved ? (
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        {selectedInscription.admin_approved_at && (
                                                            <span className="text-xs text-slate-600">le {new Date(selectedInscription.admin_approved_at).toLocaleDateString("fr-FR")}</span>
                                                        )}
                                                    </div>
                                                ) : (
                                                    <div className="flex items-center gap-2">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        ...
                                        {selectedInscription.updated_at && (
                                            <div>
                                                ...
                                                <p className="text-slate-800 font-medium">
                                                    {new Date(selectedInscription.updated_at).toLocaleDateString("fr-FR")}
                                                </p>
                                            </div>
                                        )}
                                    </div>
                                        ...
                                        <div className="flex-grow space-y-3">
                                            ...
                                            {selectedMember.raw?.date_naissance && (
                                                <div>
                                                    ...
                                                    <p className="text-slate-800 font-medium">
                                                        {new Date(selectedMember.raw.date_naissance).toLocaleDateString("fr-FR")}
                                                    </p>
                                                </div>
                                            )}
                                        </div>
                                ...
                                <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div>
                                            ...
                                            <p className="text-slate-800 font-medium">
                                                {selectedMember.raw?.created_at ? new Date(selectedMember.raw.created_at).toLocaleDateString("fr-FR") : "Non disponible"}
                                            </p>
                                        </div>
                                        <div>
                                            <label className="block text-xs font-semibold text-slate-400 uppercase mb-1">
                                            ...
                                            <p className="text-slate-800 font-medium">
                                                {selectedMember.raw?.updated_at ? new Date(selectedMember.raw.updated_at).toLocaleDateString("fr-FR") : "Non disponible"}
                                            </p>
                                        </div>
                                    </div>
                                </div>
