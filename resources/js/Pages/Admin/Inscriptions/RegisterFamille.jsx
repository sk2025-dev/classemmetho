...
export default function RegisterFamille({
    ...
    const [classesSearchTerm, setClassesSearchTerm] = usePersistentState('registerFamille_classesSearchTerm', "");
...
if (membreTemp.email && !/^\S+@\S+\.\S+$/.test(membreTemp.email)) newErrors["membre.email"] = "Adresse email invalide";
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const res = await axios.post("/admin/inscriptions/famille", formData, {
                headers: headers,
            });
    ...
    const renderStepForm = () => {
        switch (step) {
                                ...
                                <div className="relative">
                                    <div className="w-20 h-20 rounded-full bg-white overflow-hidden border-3 border-blue-400 shadow-lg ring-3 ring-blue-100">
                                        {responsable.photoPreview ? (
                                            ...
                                            <div className="w-full h-full bg-gradient-to-br from-yellow-300 to-amber-400 flex items-center justify-center font-bold text-slate-700 text-2xl">
                                                {responsable.prenom?.[0]?.toUpperCase()}{responsable.nom?.[0]?.toUpperCase()}
                                            </div>
                                        )}
                                    </div>
                                </div>
                            ...
                            <FormField label="Fonction dans l'église" icon={Users} hint="Cliquez pour sélectionner">
                                <Select2Fonction
                                    ...
                                    onChange={(e) => {
                                        ...
                                        // Also set fonction_id (first selected value for single ID)
                                        const firstSelectedId = e.target.value.length > 0 ? parseInt(e.target.value[0], 10) : null;
                                        const selectedFunction = firstSelectedId ? churchRoles.find(f => f.id === firstSelectedId) : null;
                                        setResponsable({
                                            ...responsable,
                                            fonction_id: firstSelectedId,
                                            fonction: selectedFunction ? selectedFunction.nom : "",
                                        });
                                        setResponsable({
                                            ...responsable,
            ...
            case 3:
                // TOUJOURS afficher la question + le formulaire si "Oui"
                return (
                    <div className="space-y-6 animate-fadeIn">
                        {/* SECTION 1: Question Oui/Non - TOUJOURS VISIBLE */}
                        <div className="text-center space-y-4 p-6 bg-blue-50 rounded-xl border-2 border-blue-200">
                            ...
                            <p className="text-sm text-gray-600">
                                Vous pouvez ajouter des enfants, conjoints, parents ou autres membres.
                            </p>
                            ...
                            <div className="flex gap-4 justify-center pt-2">
                                <button
                                    ...
                                    className={hasMembersToAdd === false ? "px-8 py-3 rounded-lg bg-red-500 text-white font-semibold shadow-lg ring-2 ring-red-300 transition-all transform scale-105" : "px-8 py-3 rounded-lg bg-gray-200 text-gray-700 font-semibold hover:bg-gray-300 transition-all"}
                                >
                                ...
                                <button
                                    ...
                                    className={hasMembersToAdd === true ? `${STYLES.button.primary} shadow-lg ring-2 ring-blue-300 transform scale-105` : STYLES.button.primary}
                                >
                        ...
                        {/* SECTION 2: Formulaire si "Oui" est sélectionné */}
                        {hasMembersToAdd === true && (
                            <div className="space-y-6 p-6 bg-white rounded-xl border border-gray-300 shadow-md">
                                ...
                                <div className="w-full p-6 bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl border-2 border-blue-200">
                                    <div className="flex flex-col items-center gap-3">
                                        <h5 className="text-sm font-bold text-gray-800">Photo du membre</h5>
                                        <div className="w-20 h-20 rounded-full bg-white overflow-hidden border-3 border-blue-400 shadow-lg">
                                            {membreTemp.photoPreview ? (
                                                <img src={membreTemp.photoPreview} alt="profil" className="w-full h-full object-cover" />
                                            ) : (
                                                <div className="w-full h-full bg-gradient-to-br from-yellow-300 to-amber-400 flex items-center justify-center font-bold text-slate-700 text-2xl">
                                                    {membreTemp.prenom?.[0]?.toUpperCase()}{membreTemp.nom?.[0]?.toUpperCase()}
                                                </div>
                                            )}
                                        </div>
                                        <input
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <FormField label="Nom" icon={User} required>
                                        <input className={`${STYLES.input} uppercase`} value={membreTemp.nom} onChange={(e) => setMembreTemp({...membreTemp, nom: e.target.value})} placeholder="Nom de famille" />
                                        {getFieldError("membre.nom") && <p className="text-red-500 text-xs mt-1">{getFieldError("membre.nom")}</p>}
                                    </FormField>
                                    <FormField label="Prénom" icon={User} required>
                                        <input className={`${STYLES.input} capitalize`} value={membreTemp.prenom} onChange={(e) => setMembreTemp({...membreTemp, prenom: e.target.value})} placeholder="Prénom" />
                                        {getFieldError("membre.prenom") && <p className="text-red-500 text-xs mt-1">{getFieldError("membre.prenom")}</p>}
                                    </FormField>
                                </div>

                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <FormField label="Genre" icon={Users} required>
                                        <select className={STYLES.input} value={membreTemp.genre} onChange={(e) => setMembreTemp({...membreTemp, genre: e.target.value})}>
                                            <option value="">Sélectionner...</option>
                                            <option value="M">Masculin</option>
                                            <option value="F">Féminin</option>
                                        </select>
                                        {getFieldError("membre.genre") && <p className="text-red-500 text-xs mt-1">{getFieldError("membre.genre")}</p>}
                                    </FormField>
                                    <FormField label="Date de naissance" icon={Calendar} required>
                                        <input type="date" className={STYLES.input} value={membreTemp.dateNaissance} onChange={(e) => setMembreTemp({...membreTemp, dateNaissance: e.target.value})} />
                                        {getFieldError("membre.dateNaissance") && <p className="text-red-500 text-xs mt-1">{getFieldError("membre.dateNaissance")}</p>}
                                    </FormField>
                                </div>

                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <FormField label="Profession" icon={Briefcase} required>
                                        <input className={STYLES.input} value={membreTemp.profession} onChange={(e) => setMembreTemp({...membreTemp, profession: e.target.value})} placeholder="Ex: Enseignant..." />
                                        {getFieldError("membre.profession") && <p className="text-red-500 text-xs mt-1">{getFieldError("membre.profession")}</p>}
                                    </FormField>
                                    <FormField label="Lien de parenté" icon={Users} required>
                                        <Select2Relation value={membreTemp.relation} onChange={(e) => setMembreTemp({...membreTemp, relation: e.target.value})} placeholder="Sélectionner..." />
                                        {getFieldError("membre.relation") && <p className="text-red-500 text-xs mt-1">{getFieldError("membre.relation")}</p>}
                                    </FormField>
                                </div>
                                ...
                                <div className="border-t pt-4 mt-4">
                                    <FormField label="Statut Marital" icon={Heart} required>
                                        ...
                                        >
                                            ...
                                            <option value="divorce">Divorcé(e)</option>
                                            <option value="veuf">Veuf(ve)</option>
                                            <option value="dot">Dot (Cadeau de mariage)</option>
                                        </select>
                                ...
                                {/* Informations Religieuses - Membres */}
                                <div className="border-t pt-6 mt-6">
                                            ...
                                            <input
                                                ...
                                                onChange={(e) =>
                                                    setMembreTemp({
                                                        ...membreTemp,
                                                        baptise: e.target.checked,
                                                        dateBapteme: e.target.checked ? membreTemp.dateBapteme : "",
                                                        lieuBapteme: e.target.checked ? membreTemp.lieuBapteme : "",
                                                    })
                                                }
                                                className="w-5 h-5 rounded border-gray-300 text-blue-600"
                                            />
                                    ...
                                    <div className="space-y-3 mb-4">
                                            ...
                                            <input
                                                ...
                                                onChange={(e) =>
                                                    setMembreTemp({
                                                        ...membreTemp,
                                                        premiereCommunion: e.target.checked,
                                                        datePremiereCommunion: e.target.checked ? membreTemp.datePremiereCommunion : "",
                                                        lieuPremiereCommunion: e.target.checked ? membreTemp.lieuPremiereCommunion : "",
                                                    })
                                                }
                                                className="w-5 h-5 rounded border-gray-300 text-purple-600"
                                            />
                                        ...
                                        {membreTemp.premiereCommunion === true && (
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 ml-8 mt-3">
                                                <FormField label="Date de la première communion">
                                                    <input
                                                        type="date"
                                    ...
                                    <div className="space-y-3">
                                            ...
                                            <input
                                                ...
                                                onChange={(e) =>
                                                    setMembreTemp({
                                                        ...membreTemp,
                                                        marieReligieusement: e.target.checked,
                                                        dateMariageReligieux: e.target.checked ? membreTemp.dateMariageReligieux : "",
                                                        lieuMariageReligieux: e.target.checked ? membreTemp.lieuMariageReligieux : "",
                                                    })
                                                }
                                                className="w-5 h-5 rounded border-gray-300 text-pink-600"
                                            />
                                        ...
                                        {membreTemp.marieReligieusement === true && (
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 ml-8 mt-3">
                                                <FormField label="Date du mariage religieux">
                                                    <input
                                                        type="date"
                        ...
                        {/* SECTION 3: Liste des membres ajoutés */}
                        {membres.length > 0 && (
                            <div className="p-4 bg-green-50 border border-green-200 rounded-lg">
                                <h4 className="font-semibold text-green-800 mb-3">Membres ajoutés ({membres.length})</h4>
                                <ul className="space-y-2">
                                    {membres.map((m, idx) => (
                                        <li key={idx} className="flex items-center justify-between p-3 bg-white rounded border border-green-200">
                                            <span className="text-sm text-gray-800">{m.prenom} {m.nom} - {m.relation}</span>
                                            <button type="button" onClick={() => supprimerMembre(idx)} className="text-red-600 hover:text-red-800 p-1">
                        ...
                        {/* SECTION 4: Message d'erreur si "Oui" mais pas de membres */}
                        {hasMembersToAdd === true && membres.length === 0 && (
                            <div className="p-4 bg-yellow-50 border-l-4 border-yellow-400 rounded">
                                <p className="text-yellow-800 font-medium">⚠️ Veuillez ajouter au moins un membre avant de continuer</p>
                            </div>
                        )}
                    </div>
                );
