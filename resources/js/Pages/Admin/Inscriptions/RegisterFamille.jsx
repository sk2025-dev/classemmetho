...
export default function RegisterFamille({
    ...
    const supprimerMembre = (index) => {
        const membreName = `${membres[index].prenom} ${membres[index].nom}`;
        if (
            window.confirm(`Êtes-vous sûr de vouloir supprimer ${membreName} ?`)
        ) {
            const updatedMembres = membres.filter((_, i) => i !== index);
            setMembres(updatedMembres);
            showSuccess(`✅ ${membreName} a été supprimé(e).`);
        }
    };
    ...
    const handleSubmit = async (e) => {
        ...
        if (responsable.statutMarital === "marie") {
            if (!responsable.dateMariage)
                validationErrors["responsable.dateMariage"] =
                    "Date du mariage requise";
            if (!responsable.lieuMariage)
                validationErrors["responsable.lieuMariage"] =
                    "Lieu du mariage requis";
        }
        if (responsable.statutMarital === "divorce") {
            if (!responsable.dateDivorce)
                validationErrors["responsable.dateDivorce"] =
                    "Date du divorce requise";
            if (!responsable.lieuDivorce)
                validationErrors["responsable.lieuDivorce"] =
                    "Lieu du divorce requis";
        }
        if (responsable.statutMarital === "veuf") {
            if (!responsable.dateDeces)
                validationErrors["responsable.dateDeces"] =
                    "Date du décès requise";
            if (!responsable.lieuDeces)
                validationErrors["responsable.lieuDeces"] =
                    "Lieu du décès requis";
        }
        if (responsable.statutMarital === "dot") {
            if (!responsable.dote)
                validationErrors["responsable.dote"] =
                    "Date de la dote requise";
            if (!responsable.lieuDote)
                validationErrors["responsable.lieuDote"] =
                    "Lieu de la dote requis";
        }
        ...
        // 🔍 LOG: Afficher tous les champs envoyés
        ...
        try {
            ...
            // 🔍 LOG: Afficher les erreurs exactes du serveur COMPLÈTEMENT
            console.error(
                "❌ ERREURS DE VALIDATION DU SERVEUR - Status:",
                err?.response?.status,
            );
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <FormField label="Niveau d'étude" icon={Briefcase} required>
                                    <input
                                        ...
                                        onChange={(e) => setResponsable({ ...responsable, niveau_etude: e.target.value })}
                                        placeholder="ex: Licence, Master, BTS..."
                                    />
                                    {getFieldError("responsable.niveau_etude") && (
                                        <p className="text-red-500 text-xs mt-1">{getFieldError("responsable.niveau_etude")}</p>
                                    )}
                                </FormField>
                            </div>
                        )}
                                ...
                                <button
                                    ...
                                    className={
                                        hasMembersToAdd === false
                                            ? "px-8 py-3 rounded-lg bg-red-500 text-white font-semibold shadow-lg ring-2 ring-red-300 transition-all transform scale-105"
                                ...
                                <button
                                    ...
                                    className={
                                        hasMembersToAdd === true
                                            ? `${STYLES.button.primary} shadow-lg ring-2 ring-blue-300 transform scale-105`
                        ...
                        {hasMembersToAdd === true && (
                            <div className="space-y-6 p-6 bg-white rounded-xl border border-gray-300 shadow-md">
                                ...
                                {membreTemp.employment_status === "ETUDIANT" && (
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <FormField label="Niveau d'étude" icon={Briefcase} required>
                                            <input className={STYLES.input} value={membreTemp.niveau_etude || ""}
                                                onChange={(e) => setMembreTemp({ ...membreTemp, niveau_etude: e.target.value })}
                                                placeholder="ex: Licence, Master, BTS..." />
                                            {getFieldError("membre.niveau_etude") && (
                                                <p className="text-red-500 text-xs mt-1">{getFieldError("membre.niveau_etude")}</p>
                                            )}
                                        </FormField>
                                    </div>
                                )}
                                ...
                                {/* Champs conditionnels pour mariage/divorce/veuvage */}
                                    ...
                                    <div className="mb-4">
                                        ...
                                        {membreTemp.premiereCommunion && (
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-3 ml-6">
                                                ...
                                                <FormField
                                                    label="Lieu de la première communion"
                                                    icon={Building}
                                                >
                                                    <input
                                                        className="w-full h-10 border border-gray-300 rounded-lg px-3 outline-none"
                        ...
                        {membres.length > 0 && (
                            <div className="space-y-3">
                                ...
                                {membres.map((m, idx) => (
                                    <div
                                        key={idx}
                                        className="p-4 bg-gradient-to-r from-white to-green-50 border border-green-200 rounded-xl shadow-sm hover:shadow-lg transition-all duration-300"
                                    >
                                                    ...
                                                    <div className="text-sm text-gray-600 space-y-1 mt-2">
                                                        ...
                                                        {m.premiereCommunion && (
                                                            <div className="text-xs text-blue-600">
                                                                Première
                                                                communion
                                                                {m.datePremiereCommunion &&
                                                                    ` le ${m.datePremiereCommunion}`}
                                                                {m.lieuPremiereCommunion &&
                                                                    ` à ${m.lieuPremiereCommunion}`}
                                                            </div>
                                                        )}
                                                        {m.marieReligieusement && (
                                                            <div className="text-xs text-red-600">
                                                                Marié
                                                                religieusement
                                                                {m.dateMariageReligieux &&
                                                                    ` le ${m.dateMariageReligieux}`}
                                                                {m.lieuMariageReligieux &&
                        ...
                        {hasMembersToAdd === true && membres.length === 0 && (
                            <div className="p-4 bg-yellow-50 border-l-4 border-yellow-400 rounded">
                                <p className="text-yellow-800 font-medium">
                                    ⚠️ Veuillez ajouter au moins un membre avant
                                    de continuer
                                </p>
                            </div>
                        )}
