...
export default function Programmes() {
  const { props } = usePage();
  const {
    initialParishSchedule = [],
    initialClassSchedule = [],
    initialParishList = [],
    initialClassList = [],
    initialParishHistory = [],
    initialClassHistory = [],
  } = props;
  ...
  const [parishList, setParishList] = useState(initialParishList);
  const [classList, setClassList] = useState(initialClassList);
  const [parishHistory, setParishHistory] = useState(initialParishHistory);
  ...
  const handleSaveAgendaModal = async (data) => {
    try {
      await axios.post('/admin/programmes/agenda', {
        context: agendaModalContext,
        ...data
      });
      router.reload(); // recharge la page pour afficher les nouvelles données
    } catch (error) {
  ...
  const handleSaveEventModal = async (data) => {
    try {
      await axios.post('/admin/programmes/event', {
      ...
      <div className="min-h-screen animate-fade-in-up" style={{ background: "linear-gradient(135deg, #6B46C1 0%, #1E40AF 50%, #B6C01A 100%)", paddingBottom: '40px' }}>
        <main style={{ padding: '0 15px' }}>
          ...
          {activeTab === 'parish' && (
            <div className="tab-content active">
              <div className="action-bar">
                <h1>🏛️ NOS ACTIVITES HEBDOMADAIRES</h1>
          ...
          {activeTab === 'classes' && (
            <div className="tab-content active">
              <div className="action-bar">
                <h2>🎒 NOS ACTIVITES HEBDOMADAIRES</h2>
