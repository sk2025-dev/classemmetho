...
export default function AdminPage({
    ...
    const [selectedForValidation, setSelectedForValidation] = React.useState(null);
    const [isValidationModalOpen, setIsValidationModalOpen] = React.useState(false);
    const [showToggleClasseModal, setShowToggleClasseModal] = React.useState(false);
    const [selectedClasseForToggle, setSelectedClasseForToggle] = React.useState(null);
    ...
    const [classesData, setClassesData] = React.useState(initialDataByType?.classes || []);
    ...
    const handleDeleteClass = (classe) => {
        return new Promise((resolve, reject) => {
            console.log("🗑️ handleDeleteClass appelé pour:", classe.nom, "ID:", classe.id);
            ...
            console.log("✅ Classe vide, envoi DELETE request");

            // Récupérer le CSRF token
            const csrfToken = document.querySelector('meta[name="csrf-token"]')?.content;

            // Utiliser fetch() pour avoir le contrôle complet
            fetch(`/admin/classes/${classe.id}`, {
                method: 'DELETE',
                headers: {
            ...
            .then(data => {
                ...
                toast.success(`Classe "${classe.nom}" supprimée avec succès`);

                // ✅ Mettre à jour l'état React immédiatement
                setClassesData(prevClasses => prevClasses.filter(c => c.id !== classe.id));

                // Force un reload complet après 1.5s pour récupérer les données fraîches du serveur
                setTimeout(() => {
            ...
            .catch(error => {
                ...
                let errorMsg = error.message || 'Erreur lors de la suppression de la classe';
    ...
    // Handleur pour toggle status d'une classe
    const handleToggleClasseStatus = () => {
        ...
        router.patch(`/admin/classes/${selectedClasseForToggle.id}/status`, { status: newStatus }, {
            onSuccess: () => {
                toast.success(`Classe "${selectedClasseForToggle.nom}" ${textAction}e avec succès. Les membres ont été ${newStatus === 'inactive' ? 'désactivés' : 'activés'}.`);
    ...
    const handleToggleMember = (member) => {
        ...
        router.patch(`/admin/membres/${member.id}/status`, {
            statut: newStatus
        }, {
            preserveScroll: true,
            onSuccess: () => {
                const displayStatus = member.is_active ? 'désactivé' : 'activé';
    ...
    const handleStoreMembre = (newMemberData) => {
        ...
        Object.keys(newMemberData).forEach(key => {
            ...
            } else if (newMemberData[key] !== null && newMemberData[key] !== undefined) {
                formData.append(key, newMemberData[key]);
            }
        });
    ...
    const approve = () => {
        ...
        router.put(`/admin/status/${selectedForValidation.id}/inscription`,
            { status: 'Approuvé' },
            {
                onSuccess: () => {
    ...
    return (
        <div className="min-h-screen bg-gray-50">
            ...
            {/* Fond d'écran */}
            <div className="fixed inset-0 z-0" style={{ background: "linear-gradient(135deg, #6B46C1 0%, #1E40AF 50%, #B6C01A 100%)" }}></div>
            ...
            <div className="relative z-10 flex-1 flex flex-col pt-4 overflow-hidden px-4 sm:px-6 lg:px-8 pb-8">
                ...
                {activeTab === "utilisateurs" && (
                    <TabUtilisateurs
                        ...
                        membres={initialMembres}       // Liste des membres (tableau)
            ...
            <ValidationModal
                ...
                onApprove={handleToggleClasseStatus}
