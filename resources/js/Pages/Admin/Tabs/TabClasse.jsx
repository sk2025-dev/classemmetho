...
const MemberDetailsModal = ({ isOpen, onClose, member }) => {
                ...
                try {
                    ...
                    if (response.ok) {
                        ...
                        setDisplayMember({
                            ...member,
                            ...freshData,
                            relation: freshData?.relation ?? member?.relation ?? null,
                            fonction: freshData?.fonction ?? member?.fonction ?? null,
                            profession: freshData?.profession ?? member?.profession ?? member?.fonction_professionnelle ?? null,
                            quartier: freshData?.quartier ?? member?.quartier ?? null,
                            ville: freshData?.ville ?? member?.ville ?? null,
                            adresse: freshData?.adresse ?? member?.adresse ?? null,
                            identifier: freshData?.identifier ?? member?.identifier ?? member?.identifiant ?? null,
                            identifiant: freshData?.identifiant ?? member?.identifiant ?? member?.identifier ?? null,
                        });
                    } else {
    ...
    const normalizedData = {
        ...
        fonction: displayData?.fonction
            || displayData?.fonction_nom
            || (displayData?.fonction_id ? `Fonction ID: ${displayData.fonction_id}` : null)
            || displayData?.profession
            || displayData?.fonction_professionnelle
            || '-',
        profession: displayData?.profession || displayData?.fonction_professionnelle || '-',
        quartier: displayData?.quartier || displayData?.famille?.quartier || '-',
        ville: displayData?.ville
            || displayData?.ville_nom
            || displayData?.famille?.ville
            || (displayData?.famille_ville_id ? `Ville ID: ${displayData.famille_ville_id}` : null)
            || (displayData?.ville_id ? `Ville ID: ${displayData.ville_id}` : null)
            || (displayData?.famille?.ville_id ? `Ville ID: ${displayData.famille.ville_id}` : null)
            || '-',
    };
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-5">
                    ...
                    <DetailCard title="Contact" icon={<Mail className="w-5 h-5" />}>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                            ...
                            <InfoItem label="Identifiant" value={displayValue(normalizedData.identifier)} icon={<Award className="w-4 h-4 text-blue-500" />} />
                            <InfoItem label="Téléphone 2" value={displayValue(displayData.telephone2)} icon={<Phone className="w-4 h-4 text-blue-500" />} />
                            <InfoItem label="Adresse" value={displayValue(normalizedData.adresse)} icon={<MapPin className="w-4 h-4 text-blue-500" />} className="md:col-span-2" />
                        </div>
                    </DetailCard>
                    ...
                    <DetailCard title="Informations Personnelles" icon={<User className="w-5 h-5" />}>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                            <InfoItem label="Profession" value={displayValue(normalizedData.profession)} />
                            ...
                            <InfoItem label="Ville" value={displayValue(normalizedData.ville)} icon={<MapPin className="w-4 h-4 text-blue-500" />} />
                            <InfoItem label="Quartier" value={displayValue(normalizedData.quartier)} />
                        </div>
                    </DetailCard>
                    ...
                    <DetailCard title="Situation Familiale" icon={<Users className="w-5 h-5" />}>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                            ...
                            <InfoItem label="Relation" value={displayValue(normalizedData.relation)} />
                    ...
                    <DetailCard title="Paroisse & Sacrements" icon={<BookOpen className="w-5 h-5" />}>
                        <div className="space-y-5">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                                <InfoItem label="Fonction" value={displayValue(normalizedData.fonction)} />
