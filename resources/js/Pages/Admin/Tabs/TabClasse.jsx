        ...
        <div className="flex items-center justify-center gap-2">
            ...
            {onToggle && (
                <button
                    ...
                    title={
                        rowData?.status === "active" ? "Désactiver" : "Activer"
                    }
                >
                    {rowData?.status === "active" ? (
                        <svg
...
const StatusBadge = ({ isActive }) => (
    <span
        className={`px-2 py-0.5 rounded text-xs font-semibold ${isActive ? "bg-green-100 text-green-700 border border-green-200" : "bg-red-100 text-red-700 border border-red-200"}`}
    >
...
const MemberDetailsModal = ({ isOpen, onClose, member }) => {
                ...
                try {
                    ...
                    if (response.ok) {
                        ...
                        setDisplayMember({
                            ...
                            ...freshData,
                            relation:
                                freshData?.relation ?? member?.relation ?? null,
                            fonction:
                                freshData?.fonction ?? member?.fonction ?? null,
                            profession:
                                freshData?.profession ??
                                member?.profession ??
                                member?.fonction_professionnelle ??
                                null,
                            quartier:
                                freshData?.quartier ?? member?.quartier ?? null,
                            ville: freshData?.ville ?? member?.ville ?? null,
                            adresse:
                                freshData?.adresse ?? member?.adresse ?? null,
                            identifier:
                                freshData?.identifier ??
                                member?.identifier ??
                                member?.identifiant ??
                                null,
                            identifiant:
                                freshData?.identifiant ??
                                member?.identifiant ??
                                member?.identifier ??
                                null,
                        });
    ...
    const normalizedData = {
        ...
        fonction:
            displayData?.fonction ||
            displayData?.fonction_nom ||
            (displayData?.fonction_id
                ? `Fonction ID: ${displayData.fonction_id}`
                : null) ||
            displayData?.profession ||
            displayData?.fonction_professionnelle ||
            "-",
        profession:
            displayData?.profession ||
            displayData?.fonction_professionnelle ||
            "-",
        quartier:
            displayData?.quartier || displayData?.famille?.quartier || "-",
        ville:
            displayData?.ville ||
            displayData?.ville_nom ||
            displayData?.famille?.ville ||
            (displayData?.famille_ville_id
                ? `Ville ID: ${displayData.famille_ville_id}`
                : null) ||
            (displayData?.ville_id
                ? `Ville ID: ${displayData.ville_id}`
                : null) ||
            (displayData?.famille?.ville_id
                ? `Ville ID: ${displayData.famille.ville_id}`
                : null) ||
            "-",
    };
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-5">
                    ...
                    <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 flex items-start gap-6">
                        <div className="relative shrink-0">
                            {resolveMemberPhotoUrl(displayData) ? (
                                <img
                                    src={resolveMemberPhotoUrl(displayData)}
                                    alt={displayData.nom}
                    ...
                    >
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                            <InfoItem
                                label="Email"
                                value={displayValue(displayData.email)}
                                icon={
                            ...
                            <InfoItem
                                label="Téléphone"
                                value={displayValue(displayData.telephone)}
                                icon={
                            ...
                            <InfoItem
                                label="Téléphone 2"
                                value={displayValue(displayData.telephone2)}
                                icon={
                                    <Phone className="w-4 h-4 text-blue-500" />
                                }
                            />
                            <InfoItem
                                label="Adresse"
                                value={displayValue(normalizedData.adresse)}
                                icon={
                    ...
                    >
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                            <InfoItem
                                label="Profession"
                                value={displayValue(normalizedData.profession)}
                            />
                            <InfoItem
                                label="Genre"
                                value={
                            ...
                            <InfoItem
                                ...
                                value={displayValue(displayData.date_naissance)}
                                icon={
                            ...
                            <InfoItem
                                label="Ville"
                                value={displayValue(normalizedData.ville)}
                                icon={
                                    <MapPin className="w-4 h-4 text-blue-500" />
                                }
                            />
                            <InfoItem
                                label="Quartier"
                                value={displayValue(normalizedData.quartier)}
                            />
                        </div>
                    </DetailCard>
                    ...
                    >
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                            <InfoItem
                                ...
                                value={displayValue(displayData.statut_marital)}
                            />
                            <InfoItem
                                label="Relation"
                                value={displayValue(normalizedData.relation)}
                            />
                            <InfoItem
                                label="Nom de famille"
                                value={displayValue(displayData.famille_nom)}
                                className="md:col-span-2"
                            />
                        </div>
                    </DetailCard>
                    ...
                    >
                        <div className="space-y-5">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                                ...
                                <InfoItem
                                    label="Classe"
                                    value={displayValue(displayData.classe)}
                                />
                            </div>
                            <div className="border-t border-gray-200 pt-5">
                                ...
                                <div className="space-y-4">
                                    <div className="flex items-center justify-between p-3 bg-blue-50 rounded-lg border border-blue-200">
                                        ...
                                        <StatusBadgeValue
                                            value={displayData.baptise}
                                        />
                                    </div>
                                    {(displayData.baptise === "oui" ||
                                        displayData.baptise === true ||
                                        displayData.baptise === 1) && (
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pl-3 border-l-2 border-blue-300">
                                            <InfoItem
                                                label="Date"
                                                value={formatDate(
                                                    displayData.date_bapteme,
                                                )}
                                                icon={
                                                    <Calendar className="w-4 h-4 text-blue-500" />
                                                }
                                    ...
                                    <div className="flex items-center justify-between p-3 bg-yellow-50 rounded-lg border border-yellow-200">
                                        ...
                                        <StatusBadgeValue
                                            value={
                                                displayData.premiere_communion
                                            }
                                        />
                                    </div>
                                    {(displayData.premiere_communion ===
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pl-3 border-l-2 border-yellow-300">
                                            <InfoItem
                                                label="Date"
                                                value={formatDate(
                                                    displayData.date_premiere_communion,
                                                )}
                                                icon={
                                            ...
                                            <InfoItem
                                                label="Lieu"
                                                value={displayValue(
                                                    displayData.lieu_premiere_communion,
                                                )}
                                                icon={
                                    ...
                                    <div className="flex items-center justify-between p-3 bg-rose-50 rounded-lg border border-rose-200">
                                        ...
                                        <StatusBadgeValue
                                            value={
                                                displayData.marie_religieusement
                                            }
                                        />
                                    </div>
                                    {(displayData.marie_religieusement ===
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pl-3 border-l-2 border-rose-300">
                                            <InfoItem
                                                label="Date"
                                                value={formatDate(
                                                    displayData.date_marie_religieusement,
                                                )}
                                                icon={
                                            ...
                                            <InfoItem
                                                label="Lieu"
                                                value={displayValue(
                                                    displayData.lieu_marie_religieusement,
                                                )}
                                                icon={
                    ...
                    >
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                            <InfoItem
                                label="Mariage civil"
                                value={formatDate(displayData.date_mariage)}
                                icon={
                                    <Calendar className="w-4 h-4 text-blue-500" />
                                }
                            />
                            <InfoItem
                                label="Lieu"
                                value={displayValue(displayData.lieu_mariage)}
                            />
                            <div className="md:col-span-2 flex items-center justify-between p-3 bg-purple-50 rounded-lg border border-purple-200">
                                ...
                                <StatusBadgeValue
                                    value={
                                        displayData.statut_marital === "Dote"
                                    }
                                />
                            </div>
...
const EditMemberModal = ({ isOpen, onClose, memberData, onUpdate }) => {
    ...
    const [photoPreview, setPhotoPreview] = useState(
        resolveMemberPhotoUrl(safeMemberData) || null,
    );
                ...
                try {
                    ...
                    if (response.ok) {
                        ...
                        const mappedData = {
                            ...
                            statut_marital:
                                freshData.statut_marital || "Célibataire",
                            ...
                            date_marie_religieusement:
                                freshData.date_marie_religieusement || "",
                            lieu_marie_religieusement:
                                freshData.lieu_marie_religieusement || "",
    ...
    const handleSubmit = (e) => {
        ...
        const dataToSubmit = {
            ...
            date_naissance:
                formData.date_naissance || memberData?.date_naissance || "",
            ...formData,
        };
        ...
        const dateMarriageFromForm =
            dataToSubmit.date_mariage &&
            dataToSubmit.date_mariage.trim() !== "";
            ...
            <div className="bg-white/90 backdrop-blur-xl rounded-2xl shadow-2xl overflow-hidden border border-white/50 w-full max-w-6xl max-h-[90vh] flex flex-col">
                ...
                <div className="px-8 py-5 border-b border-gray-100 bg-white/50 flex justify-between items-center shrink-0">
                    <h2 className="text-xl font-bold text-gray-800">
                        Modifier : {memberData.prenom} {memberData.nom}
                    </h2>
                    <button
                        onClick={onClose}
                            ...
                            <section>
                                ...
                                <div className="space-y-3">
                                    ...
                                    <SacrementSection
                                        ...
                                        color="yellow"
                                        checked={
                                            formData.premiere_communion || false
                                        }
                                        onChange={(val) =>
                                            setFormData({
                                                ...formData,
                                            ...
                                            <>
                                                <FormField label="Date de première communion">
                                                    <input
                                                        ...
                                                        className="w-full h-10 border border-gray-300 rounded px-2 bg-white focus:border-blue-500 focus:shadow-md focus:shadow-blue-200 transition-all duration-300"
                                                        value={
                                                            formData.date_premiere_communion ||
                                                            ""
                                                        }
                                                        onChange={handleChange}
                                                ...
                                                <FormField label="Lieu de première communion">
                                                    <input
                                                        className="w-full h-10 border border-gray-300 rounded px-2 focus:border-blue-500 focus:shadow-md focus:shadow-blue-200 transition-all duration-300"
                                                        value={
                                                            formData.lieu_premiere_communion ||
                                                            ""
                                                        }
                                                        onChange={handleChange}
                                            ...
                                            <>
                                                <FormField label="Date du mariage religieux">
                                                    <input
                                                        ...
                                                        className="w-full h-10 border border-gray-300 rounded px-2 bg-white focus:border-blue-500 focus:shadow-md focus:shadow-blue-200 transition-all duration-300"
                                                        value={
                                                            formData.date_mariage_religieux ||
                                                            ""
                                                        }
                                                        onChange={handleChange}
                                                ...
                                                <FormField label="Lieu du mariage religieux">
                                                    <input
                                                        className="w-full h-10 border border-gray-300 rounded px-2 focus:border-blue-500 focus:shadow-md focus:shadow-blue-200 transition-all duration-300"
                                                        value={
                                                            formData.lieu_mariage_religieux ||
                                                            ""
                                                        }
                                                        onChange={handleChange}
...
const ClasseDetailsModal = ({
    ...
    const membresEnrichis = membresDeLaClasse.map((membre) => {
        const familleMembre = familles.find(
            (f) => f.id === membre.famille_id || f.nom === membre.famille_nom,
        );
        return {
            ...membre,
            famille_nom:
                familleMembre?.nom || membre.famille_nom || "Non assignée",
        };
    });

    const famillesList = [
        "tous",
        ...new Set(membresEnrichis.map((m) => m.famille_nom).filter(Boolean)),
    ];
    ...
    const filteredMembres = membresEnrichis.filter((membre) => {
        ...
        const textMatch =
            ...
            membre.prenom?.toLowerCase().includes(term) ||
            membre.email?.toLowerCase().includes(term) ||
            membre.famille_nom?.toLowerCase().includes(term);
        const familleMatch =
            familleFilter === "tous" || membre.famille_nom === familleFilter;
        const baptiseMatch =
            baptiseFilter === "tous" ||
            (baptiseFilter === "oui" && membre.baptise === true) ||
            (baptiseFilter === "non" && membre.baptise !== true);
        const statutMatch =
            statutMaritalFilter === "tous" ||
            (statutMaritalFilter === "marie" &&
                membre.statut_marital === "Marié(e)") ||
            (statutMaritalFilter === "celibataire" &&
                membre.statut_marital !== "Marié(e)");
                        ...
                        <div className="flex flex-wrap gap-3">
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <select
                                    ...
                                    className="px-4 py-2 rounded-lg bg-gray-50 border border-gray-200 text-sm outline-none focus:ring-2 focus:ring-blue-500"
                                >
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <select
                                    value={baptiseFilter}
                                    onChange={(e) =>
                                        setBaptiseFilter(e.target.value)
                                    }
                                    className="px-4 py-2 rounded-lg bg-gray-50 border border-gray-200 text-sm outline-none focus:ring-2 focus:ring-blue-500"
                                >
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <select
                                    value={statutMaritalFilter}
                                    onChange={(e) =>
                                        setStatutMaritalFilter(e.target.value)
                                    }
                                    className="px-4 py-2 rounded-lg bg-gray-50 border border-gray-200 text-sm outline-none focus:ring-2 focus:ring-blue-500"
                                >
                                    <option value="tous">Tous</option>
                                ...
                                <tbody className="divide-y divide-gray-200 bg-white/60 backdrop-blur-sm">
                                    {filteredMembres.length > 0 ? (
                                        filteredMembres.map((m, idx) => (
                                            ...
                                            >
                                                <td className="px-3 py-3 whitespace-nowrap text-sm font-bold text-gray-800 text-center">
                                                    {idx + 1}
                                                </td>
                                                ...
                                                <td className="px-3 py-3 whitespace-nowrap text-sm text-gray-600 text-center text-xs">
                                                    {m.email || "-"}
                                                </td>
                                                <td className="px-3 py-3 whitespace-nowrap text-sm text-gray-600 text-center">
        ...
        action(url, {
            onSuccess: () => {
                const message = isEditing
                    ...
                    : `Classe "${data.nom}" créée avec succès !`;
...
const TabClasses = ({
    ...
    const enrichedClassesData = classesData.map((classe) => {
        ...
        const famillesInClass = famillesData.filter(
            (f) => f.classe_id === classe.id || f.classe_nom === classe.nom,
        );
        // Ne compter que les VRAIS MEMBRES, pas les admins/conducteurs/fonctions
        const membresInClass = membresData.filter((m) => {
            const isInClass =
                m.classe_id === classe.id || m.classe_nom === classe.nom;
            const isNotAdmin =
                m.role !== "admin" &&
                m.role !== "conducteur" &&
                m.role !== "fonction";
        ...
        return {
            ...
            created_at: classe.created_at || null,
            updated_at: classe.updated_at || null,
        };
    });
    ...
    const stats = {
        ...
        totalMembres: membresData.filter(
            (m) =>
                m.role !== "admin" &&
                m.role !== "conducteur" &&
                m.role !== "fonction",
        ...
        totalConducteurs: classesData.reduce(
            (acc, c) => acc + (c.conducteurs_ids?.length || 0),
            0,
        ),
    };

    const openDeleteAlert = (classe) => {
        setSelectedClasseForDelete(classe);
        ...
        router.patch(
            ...
            { status: newStatus },
            {
                onSuccess: (response) => {
                    ...
                    toast.success(
                        `Classe "${selectedClasseForToggle.nom}" ${textAction}e avec succès.`,
                    );
    ...
    const handleMemberToggleConfirm = () => {
        ...
        router.patch(
            withBasePath("", `/admin/membres/${selectedMember.id}/status`),
            {
                statut: newStatus,
            },
            {
                preserveScroll: true,
                onSuccess: () => {
                    const actionMsg = selectedMember.is_active
                        ? "désactivé"
                        : "activé";
                    toast.success(
                        `Membre "${memberName}" ${actionMsg} avec succès`,
                    );
    ...
    const handleMemberDeleteConfirm = () => {
        if (!selectedMember) return;
        const memberName = `${selectedMember.prenom} ${selectedMember.nom}`;
        router.delete(withBasePath("", `/admin/membres/${selectedMember.id}`), {
            preserveScroll: true,
            onSuccess: () => {
    ...
    const handleMemberEdit = (updatedMember) => {
        ...
        Object.keys(updatedMember).forEach((key) => {
            if (key === "photo" && updatedMember[key] instanceof File) {
                ...
                updatedMember[key] !== null &&
                updatedMember[key] !== undefined
            ) {
                formData.append(key, updatedMember[key]);
            }
        });
