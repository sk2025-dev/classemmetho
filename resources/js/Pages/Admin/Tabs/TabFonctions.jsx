...
const ActionButtonsSVG = ({ rowData, onEdit, onDelete, onView, count }) => {
    return (
        <div className="flex items-center justify-center gap-2">
            <button onClick={onView} className="btn-icon" style={{ backgroundColor: "rgba(37, 99, 235, 0.1)", border: "2px solid rgba(37, 99, 235, 0.2)" }} title={`Voir les ${count} membres`}>
...
const FonctionMembersModal = ({ isOpen, onClose, title, members = [], onViewProfile }) => {
    const [search, setSearch] = useState("");
    if (!isOpen) return null;

    const filteredMembers = members.filter(m => {
        const term = search.toLowerCase();
        return (m.nom?.toLowerCase().includes(term)) || (m.prenom?.toLowerCase().includes(term)) || (m.email?.toLowerCase().includes(term));
    });
            ...
            <div className="bg-white w-full max-w-6xl rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh] border border-gray-100">
                ...
                <div className="px-8 py-5 border-b border-gray-100 bg-white/50 flex justify-between items-center shrink-0">
                    ...
                    <div className="flex items-center gap-4">
                        <div className="text-sm font-medium text-gray-600">{filteredMembers.length} / {members.length}</div>
                        ...
                        <div className="table-scroll h-full">
                            <table className="min-w-full divide-y divide-gray-200">
                                <thead className="bg-gradient-to-r from-blue-600 to-blue-700">
                                    <tr>
                                        <th className="px-4 py-3 text-xs font-extrabold text-white uppercase tracking-wider text-center whitespace-nowrap">Nom</th>
                                        <th className="px-4 py-3 text-xs font-extrabold text-white uppercase tracking-wider text-center whitespace-nowrap">Prénom</th>
                                        <th className="px-4 py-3 text-xs font-extrabold text-white uppercase tracking-wider text-center whitespace-nowrap">Email</th>
                                        <th className="px-4 py-3 text-xs font-extrabold text-white uppercase tracking-wider text-center whitespace-nowrap">Téléphone</th>
                                        <th className="px-4 py-3 text-xs font-extrabold text-white uppercase tracking-wider text-center whitespace-nowrap">Classe</th>
                                ...
                                <tbody className="divide-y divide-gray-200 bg-white/60 backdrop-blur-sm">
                                    {filteredMembers.length > 0 ? filteredMembers.map((m, idx) => (
                                        <tr key={idx} className="hover:bg-white/90 transition-all duration-200">
                                            <td className="px-4 py-3 whitespace-nowrap text-sm font-bold text-gray-800 text-center">{m.nom}</td>
                                            <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-700 text-center">{m.prenom}</td>
                                            <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-600 text-center">{m.email || '-'}</td>
                                            <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-600 text-center">{m.telephone || '-'}</td>
                                            <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-600 text-center">{m.classe || '-'}</td>
                                            <td className="px-4 py-3 whitespace-nowrap text-sm text-center">
                                                <button className="btn-icon text-blue-600" style={{ backgroundColor: "rgba(37, 99, 235, 0.1)" }} title="Profil" onClick={() => onViewProfile?.(m)}>Profil</button>
                                            </td>
                                        </tr>
                                    )) : <tr><td colSpan={6} className="px-6 py-12 text-center text-gray-400">Aucun membre trouvé.</td></tr>}
                                </tbody>
                            </table>
                        </div>
                    </div>
        ...
        <div className="fixed inset-0 z-[95] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in-up">
            <div className="bg-white w-full max-w-2xl rounded-3xl shadow-2xl overflow-hidden border border-gray-100">
                <div className="px-6 py-4 border-b border-gray-100 bg-gray-50/50 flex justify-between items-center">
                    <h3 className="text-lg font-bold text-gray-800">Profil du membre</h3>
                    <button onClick={onClose} className="p-2 text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded-lg transition-all">
                        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
                    </button>
                </div>
                <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                    <div>
                        ...
                        <p className="font-semibold text-gray-900">{member.nom || "-"}</p>
                    </div>
                    <div>
                        <p className="text-gray-500">Prénom</p>
                        <p className="font-semibold text-gray-900">{member.prenom || "-"}</p>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <p className="font-semibold text-gray-900">{member.telephone || "-"}</p>
                    </div>
                    <div>
                        <p className="text-gray-500">Classe</p>
                        <p className="font-semibold text-gray-900">{member.classe || "-"}</p>
                    </div>
                    <div>
...
const FonctionFormModal = ({ isOpen, onClose, fonctionData, onSuccess }) => {
    ...
    const handleSubmit = (e) => {
        ...
        const url = isEditing ? `/admin/fonctions/${fonctionData.id}` : "/admin/fonctions";
        action(url, {
            onSuccess: () => {
                onClose();
            ...
            <div className="bg-white w-full max-w-lg rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh] border border-gray-100">
                <div className="px-6 py-4 border-b border-gray-100 bg-gray-50/50 flex justify-between items-center shrink-0">
                    <h2 className="text-xl font-bold text-gray-800 flex items-center gap-3">
                        ...
                        {isEditing ? "Modifier la fonction" : "Créer une fonction"}
                    </h2>
                    <button onClick={onClose} className="p-2 text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded-lg transition-all">
                        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
                    </button>
                ...
                <div className="overflow-y-auto p-6 bg-white">
                    <form onSubmit={handleSubmit} className="space-y-5">
                        <div className="form-group">
                            <label className="form-label">Nom de la fonction <span className="text-red-500 ml-1">*</span></label>
                            <input type="text" value={data.nom} onChange={e => setData('nom', e.target.value)} className={`input-control ${errors.nom ? 'border-red-500' : ''}`} placeholder="Ex: Diacre, Ancien" />
                            {errors.nom && <p className="text-red-600 text-xs mt-1">{errors.nom}</p>}
                        </div>
                        <div className="form-group">
                            <label className="form-label">Description</label>
                            <textarea rows="4" value={data.description} onChange={e => setData('description', e.target.value)} className="input-control resize-none" placeholder="Décrivez les responsabilités..."></textarea>
                        </div>
                    </form>
                </div>
...
const TabFonctions = ({ rawData, onDelete, onUpdate }) => {
    ...
    const [selectedFonctionForDelete, setSelectedFonctionForDelete] = useState(null);
    ...
    const stats = {
        ...
        totalMembersAssigned: enrichedData.reduce((acc, f) => acc + f.membres_count, 0)
    };

    const openDeleteAlert = (fonction) => { setSelectedFonctionForDelete(fonction); setShowDeleteAlert(true); };
    const handleDeleteFonction = () => {
        if (!selectedFonctionForDelete) return;
        router.delete(`/admin/fonctions/${selectedFonctionForDelete.id}`, {
            onSuccess: () => {
            ...
            <div className="flex flex-col h-full animate-fade-in-up">
                ...
                <div className="stats-grid">
                    ...
                    <div className="stat-card">
                        ...
                        <span className="stat-value">{stats.totalMembersAssigned}</span>
                ...
                <div className="glass-panel filters-bar">
                    <div className="filter-group w-full">
                        <div className="input-search-wrapper w-full md:w-80">
                            ...
                            <input type="text" placeholder="Rechercher une fonction..." className="input-control input-search" value={search} onChange={e => setSearch(e.target.value)} />
                        </div>
                    ...
                    <div className="table-scroll h-full">
                        <table className="min-w-full divide-y divide-gray-100">
                            <thead className="sticky top-0 z-10">
                                <tr>
                                    <th className="px-4 py-3 text-xs font-extrabold text-white uppercase tracking-wider text-center whitespace-nowrap">ID</th>
                                    <th className="px-4 py-3 text-xs font-extrabold text-white uppercase tracking-wider text-center whitespace-nowrap">Nom</th>
                            ...
                            <tbody className="divide-y divide-gray-100 bg-white/60 backdrop-blur-sm">
                                {currentItems.length > 0 ? currentItems.map((row) => (
                                    <tr key={row.id} className="hover:bg-white/90 transition-all duration-200">
                                        <td className="px-4 py-3 whitespace-nowrap text-sm font-bold text-gray-900 text-center">#{row.id}</td>
                                        <td className="px-4 py-3 whitespace-nowrap text-sm font-bold text-gray-900 text-center">
                                            <button onClick={() => setSelectedFonction(row.nom)} className="text-blue-600 hover:text-blue-900 hover:underline">{row.nom}</button>
                                        </td>
                                        ...
                                        <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-700 text-center">
                                            <span className="status-badge" style={{
                                                backgroundColor: row.membres_count > 0 ? '#dcfce7' : '#f3f4f6',
                                                color: row.membres_count > 0 ? '#166534' : '#6b7280',
                                                borderColor: row.membres_count > 0 ? '#bbf7d0' : '#d1d5db'
                                            }}>
                                                {row.membres_count}
                                            </span>
                                        </td>
                                        <td className="px-4 py-3 whitespace-nowrap text-sm text-center">
                                            <div className="flex items-center justify-center gap-2">
                                                <ActionButtonsSVG
                                                    rowData={row}
                                                    onEdit={() => setEditingFonction(row)}
                                                    onDelete={() => openDeleteAlert(row)}
                                                    onView={() => setSelectedFonction(row.nom)}
                                                    count={row.membres_count}
                                                />
                                            </div>
                                        </td>
                                    </tr>
                                )) : (
                                    <tr><td colSpan={5} className="px-6 py-12 text-center text-gray-400 italic">Aucune fonction trouvée.</td></tr>
                                )}
                            </tbody>
                        </table>
                    </div>
                ...
                <FonctionMembersModal
                    ...
                    members={enrichedData.find(f => f.nom === selectedFonction)?.members_list || []}
