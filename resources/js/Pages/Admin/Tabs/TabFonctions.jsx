...
// --- Modal Détails des Membres d'une Fonction (similaire à ClasseDetailsModal) ---
const FonctionDetailsModal = ({ isOpen, onClose, fonction, membres = [] }) => {
    const [search, setSearch] = useState("");
    const [baptiseFilter, setBaptiseFilter] = useState("tous");
    const [statutMaritalFilter, setStatutMaritalFilter] = useState("tous");

    if (!isOpen || !fonction) return null;
    ...
    const filteredMembres = membres.filter(membre => {
        ...
        return textMatch && baptiseMatch && statutMatch;
    });
            ...
            <div className="bg-white w-full max-w-7xl rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh] border border-gray-100">
                ...
                <div className="px-8 py-5 border-b border-gray-100 bg-white/50 flex justify-between items-center shrink-0">
                    <h2 className="text-xl font-bold text-gray-800 flex items-center gap-3">
                        ...
                        Membres de la fonction : {fonction.nom}
                    </h2>
                    <div className="flex items-center gap-4">
                        <div className="text-sm font-medium text-gray-600">{filteredMembres.length} / {membres.length}</div>
                            ...
                            <table className="min-w-full divide-y divide-gray-200">
                                <thead className="bg-gradient-to-r from-blue-600 to-blue-700">
                                    <tr>
                                        ...
                                        <th className="px-3 py-3 text-xs font-extrabold text-white uppercase tracking-wider text-center">Classe</th>
                                ...
                                <tbody className="divide-y divide-gray-200 bg-white/60 backdrop-blur-sm">
                                    {filteredMembres.length > 0 ? filteredMembres.map((m, idx) => (
                                        <tr key={idx} className="hover:bg-white/90 transition-all duration-200">
                                            ...
                                            <td className="px-3 py-3 whitespace-nowrap text-sm text-gray-600 text-center">{m.classe || "-"}</td>
                                    ...
                                    )) : <tr><td colSpan={21} className="px-6 py-12 text-center text-gray-400">Aucun membre trouvé.</td></tr>}
                                </tbody>
                            </table>
                        </div>
                    </div>
...
const FonctionFormModal = ({ isOpen, onClose, fonctionData, onSuccess, showToast }) => {
    ...
    const handleSubmit = (e) => {
        ...
        const actionType = isEditing ? 'modification' : 'création';

        const action = isEditing ? put : post;
        const url = isEditing ? `/admin/fonctions/${fonctionData.id}` : "/admin/fonctions";
        action(url, {
            onSuccess: () => {
                // Use captured nomValue instead of data.nom which might be cleared
                showToast.success(`Fonction "${nomValue}" ${actionType === 'création' ? 'créée' : 'modifiée'} avec succès`);
                onClose();
                if (onSuccess) onSuccess();
            },
            onError: (errors) => {
                showToast.error(`Erreur lors de la ${actionType} de la fonction`);
            },
            preserveScroll: true
        });
    };
...
const TabFonctions = ({ rawData, onDelete, onUpdate }) => {
    ...
    // Mettre à jour le state des toasts quand useToast génère de nouveaux toasts
    useEffect(() => {
        setToasts(showToast.toasts);
    }, [showToast.toasts]);
    ...
    const [fonctionDetails, setFonctionDetails] = useState(null);
    const [fonctionMembers, setFonctionMembers] = useState([]);
    ...
    // ✅ Charger les détails de la fonction quand elle est sélectionnée
    useEffect(() => {
        if (selectedFonction && selectedFonction.id) {
    ...
    const handleDeleteFonction = () => {
        ...
        router.delete(`/admin/fonctions/${selectedFonctionForDelete.id}`, {
            onSuccess: () => {
                ...
                const fonctionName = selectedFonctionForDelete?.nom;
                setSelectedFonctionForDelete(null);
                if (onDelete) onDelete();
                showToast.success(`Fonction "${fonctionName}" supprimée avec succès`);
                router.reload({ only: ['rawData'] });
            },
            onError: (errors) => {
                setShowDeleteAlert(false);
                showToast.error('Erreur lors de la suppression de la fonction');
            },
            preserveScroll: true,
        });
    };
    ...
    return (
        <>
            ...
            <div className="flex flex-col h-full animate-fade-in-up">
                                ...
                                {currentItems.length > 0 ? currentItems.map((row) => (
                                    <tr key={row.id} className="hover:bg-white/90 transition-all duration-200">
                                        ...
                                        <td className="px-4 py-3 whitespace-nowrap text-sm font-bold text-gray-900 text-center">
                                            <button onClick={() => setSelectedFonction(row)} className="text-blue-600 hover:text-blue-900 hover:underline">{row.nom}</button>
                                        </td>
                                        ...
                                        <td className="px-4 py-3 whitespace-nowrap text-sm text-center">
                                            <div className="flex items-center justify-center gap-2">
                                                <ActionButtonsSVG
                                                    ...
                                                    onView={() => setSelectedFonction(row)}
                ...
                <FonctionDetailsModal
                    isOpen={!!selectedFonction}
                    onClose={() => setSelectedFonction(null)}
                    fonction={fonctionDetails}
                    membres={fonctionMembers}
                />
                ...
                <ToastContainer
                    toasts={toasts}
                    removeToast={showToast.removeToast}
                />
            </div>
        </>
    );
