...
const StatusBadge = ({ role }) => {
    const isBureauConducteur = role === "bureau_conducteur";
    ...
    return (
        ...
        >
            {isBureauConducteur ? <Users size={12} /> : <CheckCircle size={12} />}
            {isBureauConducteur ? "Bureau Conducteurs" : "Membre"}
        </span>
    );
};
...
export default function Inscriptions({ family, members, familyStats, auth }) {
    ...
    const exportToCSV = () => {
        const headers = ["Nom", "Prénom", "Email", "Téléphone", "Genre", "Ville"];
        ...
        link.setAttribute(
            "download",
            `famille_${family?.nom || "export"}_${new Date().toISOString().split("T")[0]}.csv`,
        );
            ...
            <div className="mx-auto py-8 px-4 sm:px-6 lg:px-8">
                ...
                {familyStats && (
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
                        {[
                            ...
                            <div
                                key={idx}
                                className="bg-white/90 backdrop-blur-md rounded-xl p-7 shadow-lg border border-white/60 flex items-start justify-between transition-all hover:shadow-xl"
                            >
                                <div>
                ...
                <div className="bg-white/95 rounded-xl shadow-md p-5 mb-8 flex flex-col lg:flex-row gap-4 items-center justify-between border border-gray-200/50 backdrop-blur-sm">
                    ...
                    <div className="flex items-center gap-2 w-full lg:w-auto justify-end">
                        <button
                            onClick={() => router.get(withBasePath("", "/bureau-conducteur/transferts"))}
                        ...
                        <button
                            onClick={() => router.get(withBasePath("", "/bureau-conducteur/family/edit"))}
                            className="flex items-center justify-center gap-2 px-4 py-2.5 bg-gray-100 border border-gray-300 text-gray-700 font-medium rounded-lg hover:bg-gray-200 transition-all text-sm shadow-sm"
                        >
                        ...
                        <button
                            onClick={() => family && router.get(withBasePath("", "/bureau-conducteur/members/create"))}
                            disabled={!family}
                    ...
                    {(members || []).length > 0 ? (
                        <div className="overflow-x-auto">
                            <table className="w-full">
                                ...
                                >
                                    <tr>
                                        {["ID", "Nom & Photo", "Genre", "Email", "Téléphone", "Ville", "Profession", "Fonction Église", "Rôle", "Code Famille", "Code Membre", "Détails"].map((h) => (
                                            <th key={h} className={`px-6 py-4 text-${h === "Détails" ? "center" : "left"}`}>{h}</th>
                                        ))}
                                    </tr>
                                </thead>
                                <tbody>
                                    {filteredMembers.map((member, index) => (
                                        ...
                                        >
                                            <td className="px-6 py-4 text-left text-sm font-medium text-gray-600">#{member.id}</td>
                                            ...
                                            <td className="px-6 py-4 text-left text-sm"><GenreBadge genre={member.genre} /></td>
                                            <td className="px-6 py-4 text-left text-sm text-gray-600">{member.email || "—"}</td>
                                            <td className="px-6 py-4 text-left text-sm text-gray-600">{member.telephone || "—"}</td>
                                            <td className="px-6 py-4 text-left text-sm text-gray-600">{member.ville_name || "—"}</td>
                                            <td className="px-6 py-4 text-left text-sm text-gray-600">{member.profession || "—"}</td>
                                            <td className="px-6 py-4 text-left text-sm text-gray-600">{member.fonction_name || "—"}</td>
                                            <td className="px-6 py-4 text-left text-sm"><StatusBadge role={member.role} /></td>
                                            ...
                                            <td className="px-6 py-4 text-center">
                                                <div className="flex items-center justify-center gap-2">
                                                    <button
                                                        onClick={() => router.get(`/bureau-conducteur/members/${member.id}`)}
                                                    ...
                                                    <button
                                                        onClick={() => router.get(`/bureau-conducteur/members/${member.id}/edit`)}
                        ...
                        <div className="bg-white rounded-xl shadow-lg p-12 text-center">
                            ...
                            <Link
                                href={withBasePath("", `/bureau-conducteur/members/create?family_id=${family?.id || ""}`)}
