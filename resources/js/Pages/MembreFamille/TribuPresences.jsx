...
const PointageBadge = ({ statut, justifiee }) => {
    if (statut === "present") {
        return (
...
const MOTIF_OPTIONS = [
    ...
    {
        value: "empechement_professionnel",
        label: "Empêchement professionnel",
    },
    {
        value: "evenement_familial",
        label: "Date coïncide avec un événement familial",
    },
    { value: "deces_proche", label: "Décès d'un proche" },
    { value: "autre", label: "Autre" },
];

export default function TribuPresences({
    ...
    const filteredMembresActivite = useMemo(() => {
        ...
        if (!term) return membresAvecCompteurs;
        return membresAvecCompteurs.filter((m) =>
            m.nom.toLowerCase().includes(term),
        );
    }, [membresAvecCompteurs, searchMembresActivite]);

    const membresActiviteTotalPages = Math.max(
        1,
        Math.ceil(filteredMembresActivite.length / membresActivitePerPage),
    );
    ...
    const [justifyMembre, setJustifyMembre] = useState(null);
    const [justifyStep, setJustifyStep] = useState("confirm");
    const [justifyMotif, setJustifyMotif] = useState("");
    const [justifyDetail, setJustifyDetail] = useState("");
    const [sendingJustify, setSendingJustify] = useState(false);

    const openJustifyModal = (m) => {
        setJustifyMembre(m);
        setJustifyStep("confirm");
        setJustifyMotif("");
        setJustifyDetail("");
    };

    const envoyerJustification = () => {
        if (!justifyMembre || !selectedActiviteId || !justifyMotif) return;
        setSendingJustify(true);
        router.post(
            withBasePath(
                "",
                `${basePath}/tribu/presences/${selectedActiviteId}/membres/${justifyMembre.id}/justifier`,
            ),
            { motif: justifyMotif, detail: justifyDetail },
            {
                preserveScroll: true,
                onSuccess: () => setJustifyMembre(null),
                onError: (errors) =>
                    toast.error(
                        errors?.error ||
                            "Erreur lors de la justification de l'absence.",
                    ),
                onFinish: () => setSendingJustify(false),
            },
        );
    };

    const tauxMoyenNumerique = useMemo(() => {
        const avecTaux = membres.filter((m) => m.taux !== null);
        if (avecTaux.length === 0) return null;
        const moyenne =
            avecTaux.reduce((sum, m) => sum + m.taux, 0) / avecTaux.length;
        return Math.round(moyenne);
    }, [membres]);

    const tauxMoyen =
        tauxMoyenNumerique !== null ? `${tauxMoyenNumerique}%` : "-";
    ...
    // --- Tab: Marquer (détail de l'activité sélectionnée) ---
    ...
    // --- Tab: Absences (à l'activité sélectionnée) ---
    const absentsActiviteSelectionnee = useMemo(() => {
        if (!selectedActiviteId || selectedActiviteId === "all") return [];
        return membres
            .map((m) => ({
                ...m,
                presenceActivite: m.presences.find(
                    (p) => p.activite_id === selectedActiviteId,
                ),
            }))
            .filter((m) => m.presenceActivite?.statut === "absent");
    }, [membres, selectedActiviteId]);

    // Quand "Toutes les activités" est sélectionné, on retrouve le cumul
    // historique : les membres avec 3 absences ou plus, toutes activités
    // confondues (la justification n'a alors pas de sens, elle est propre à
    // une activité précise).
    const absencesRepeteesTouteActivites = useMemo(() => {
        return membres
            .map((m) => ({
                ...m,
                nbAbsences: m.presences.filter((p) => p.statut === "absent")
    ...
    const isToutesActivites = selectedActiviteId === "all";
    const absencesAffichees = isToutesActivites
        ? absencesRepeteesTouteActivites
        : absentsActiviteSelectionnee;

    const absencesTotalPages = Math.max(
        1,
        Math.ceil(absencesAffichees.length / absencesPerPage),
    );
    const paginatedAbsences = absencesAffichees.slice(
        ...
        membre.presences
            ...
            .map((p) => {
                const activite = activites.find(
                    (a) => a.id === p.activite_id,
                );
                return activite
                    ? {
                          ...activite,
                          justifiee: p.justifiee,
                          motifJustification: p.motifJustification,
                      }
                    : null;
            })
            .filter(Boolean);

    // --- Tab: Historique (journal de pointage) ---
    const filteredPointages = useMemo(() => {
        const term = search.trim().toLowerCase();
        if (!term) return pointages;
    ...
    return (
        <>
            <Head title={`Présences - ${tribu.nom}`} />
                ...
                <div className="w-full max-w-6xl mx-auto py-8">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
                        ...
                        <StatCard
                            ...
                            badge={tauxMoyen}
                            badgeClass={
                                tauxMoyenNumerique === null
                                    ? "bg-gray-100 text-gray-500"
                                    : tauxMoyenNumerique >= 75
                                      ? "bg-green-100 text-green-700"
                                      : tauxMoyenNumerique >= 40
                            ...
                            barPercent={tauxMoyenNumerique ?? 0}
                        />
                    </div>
                        ...
                        <div className="bg-white rounded-2xl shadow-xl p-6">
                            <h2 className="text-lg font-bold text-gray-900 mb-1">
                                {isToutesActivites
                                    ? "Absences répétées"
                                    : "Absents"}
                                {!isToutesActivites && activiteSelectionnee
                                    ? ` à ${activiteSelectionnee.titre}`
                                    : ""}
                            </h2>
                            {isToutesActivites ? (
                                <p className="text-sm text-gray-400 mb-4">
                                    Membres avec 3 absences ou plus, toutes
                                ...
                                activiteSelectionnee && (
                                    <p className="text-sm text-gray-400 mb-4">
                                        ...
                                        ·{" "}
                                        {absentsActiviteSelectionnee.length}{" "}
                                        absent
                                        {absentsActiviteSelectionnee.length >
                                        1
                                            ? "s"
                            ...
                            {!isToutesActivites && !activiteSelectionnee ? (
                                <p className="text-sm text-gray-400">
                                    Sélectionnez une activité pour voir ses
                                    absents.
                                </p>
                            ) : absencesAffichees.length === 0 ? (
                                <p className="text-sm text-gray-400">
                                    {isToutesActivites
                                        ? "Aucun membre n'a 3 absences ou plus pour le moment."
                                        : "Aucune absence pour cette activité. 🎉"}
                                </p>
                            ) : (
                                <>
                                    <div className="space-y-2">
                                        {isToutesActivites
                                            ...
                                            return (
                                                <div
                                                    key={m.id}
                                                    className={`flex items-center justify-between gap-4 flex-wrap p-3 rounded-xl border ${
                                                        justifiee
                    ...
                    {activeTab === "historique" && (
                        <div className="bg-white rounded-2xl shadow-xl p-6">
                            ...
                            {membres.length === 0 ? (
                                ...
                                <p className="text-sm text-gray-400">
                                    Aucun pointage ne correspond à "{search}".
                                </p>
                            ) : (
                                <>
                                    <div className="overflow-x-auto rounded-xl border border-gray-100">
                                    ...
                                    (a) => (
                                        ...
                                        >
                                            ...
                                            {a.justifiee ? (
                                                ...
                                                <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-bold bg-red-100 text-red-700 shrink-0">
                                                    Non justifiée
                                                </span>
                                            )}
                                        </li>
                        ...
                        {justifyStep === "confirm" ? (
                            <div className="p-6 space-y-5">
                                <p className="text-sm text-gray-600">
                                    ...
                                    a peut-être eu un empêchement. Voulez-vous
                                    justifier son absence à{" "}
                                ...
                                <div className="flex justify-end gap-2">
                                    ...
                                    <button
                                        ...
                                        className="px-5 py-2.5 rounded-xl text-sm font-bold bg-indigo-600 hover:bg-indigo-700 text-white"
                                    >
                                        Oui
                                    </button>
                                </div>
                            ...
                            <div className="p-6 space-y-4">
                                <div>
                                    ...
                                    <Select2Single
                                        ...
                                        value={justifyMotif}
                                        onChange={(e) =>
                                            setJustifyMotif(e.target.value)
                                        }
                                ...
                                <div>
                                    ...
                                    <textarea
                                        value={justifyDetail}
                                        onChange={(e) =>
                                            setJustifyDetail(e.target.value)
                                        }
                                        rows={3}
                                ...
                                <div className="flex justify-end gap-2 pt-2">
                                    ...
                                    <button
                                        onClick={envoyerJustification}
                                        disabled={
                                            !justifyMotif || sendingJustify
                                        }
