...
const useToast = () => {
    ...
    const success = useCallback((message, duration = 5000, action = null) => {
        return addToast(message, "success", duration, action);
    }, [addToast]);

    const error = useCallback((message, duration = 5000, action = null) => {
        return addToast(message, "error", duration, action);
    }, [addToast]);

    const warning = useCallback((message, duration = 5000, action = null) => {
        return addToast(message, "warning", duration, action);
    }, [addToast]);

    const info = useCallback((message, duration = 5000, action = null) => {
