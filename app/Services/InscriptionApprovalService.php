...
class InscriptionApprovalService
{
    ...
    private function createFamilyWithMembers(Inscription $inscription, array $data): void
    {
        ...
        $responsable->fonctions()->sync($responsableFonctionIds);
        ...
        if (!empty($membresData) && is_array($membresData)) {
            foreach ($membresData as $index => $membreData) {
                ...
                $member = User::create([
                    ...
                    'fonction_id' => $this->resolveFonctionIdsFromPayload(
                        $membreData['fonction_ids'] ?? null,
                        $membreData['fonction'] ?? null,
                        $membreData['selectedRoles'] ?? null
                    )[0] ?? null,
                ...
                $member->fonctions()->sync(
                    $this->resolveFonctionIdsFromPayload(
                        $membreData['fonction_ids'] ?? null,
                        $membreData['fonction'] ?? null,
                        $membreData['selectedRoles'] ?? null
                    )
    ...
    private function createConductorWithFamily(Inscription $inscription, array $data): void
    {
        ...
        $conductor->fonctions()->sync($responsableFonctionIds);
    ...
    private function resolveFonctionIdsFromPayload($fonctionIdsPayload, $fonctionLegacyPayload, $selectedRolesPayload = null): array
    {
        $ids = [];

        if (is_array($fonctionIdsPayload)) {
            $ids = $fonctionIdsPayload;
        } elseif (is_array($selectedRolesPayload) && !empty($selectedRolesPayload)) {
            $ids = array_map(
                fn ($role) => is_array($role) ? ($role['id'] ?? null) : $role,
                $selectedRolesPayload
            );
        } elseif (is_string($fonctionLegacyPayload) && trim($fonctionLegacyPayload) !== '') {
            $ids = preg_split('/\s*(?:,|;|\||\/)\s*/', $fonctionLegacyPayload) ?: [];
