...
class InscriptionApprovalController extends Controller
{
    ...
    {
        try {
            // ✓ Approuver par l'admin - les comptes seront créés lors de l'approbation du conducteur
            ...
            Log::info('Inscription famille approuvée par l\'admin - en attente d\'approbation conducteur', [
            ...
            return redirect()->route('admin.inscriptions')
                ->with('success', 'Famille approuvée par l\'admin. En attente d\'approbation du conducteur pour créer les comptes ✓');
    ...
    public function approveAsConductor(Request $request, Inscription $inscription)
    {
        try {
            ...
            if ($inscription->conducteur_approved_at) {
                return response()->json([
                    'success' => false,
            ...
            // Si c'est une inscription de famille, approuver aussi tous les membres
            if ($inscription->type === 'famille') {
                $membresData = $inscription->data['membres'] ?? [];
                $membresApprouves = 0;

                foreach ($membresData as $membreData) {
                    // Trouver l'inscription du membre par nom et prénom
                    $membreInscription = Inscription::where('type', 'membre_famille')
                ...
                Log::info('Inscriptions des membres de la famille approuvées par le conducteur', [
                    'famille_inscription_id' => $inscription->id,
                    'conducteur_id' => $conducteur->id,
    ...
    private function createConductorFamilyFromInscription(Inscription $inscriptionConducteur)
    {
        ...
        try {
            Mail::send(new SendCredentials($userConducteur, $identifierConducteur, $tempPasswordConducteur));
        } catch (\Exception $e) {
        ...
        if (!empty($inscriptionConducteur->data['membres']) && is_array($inscriptionConducteur->data['membres'])) {
            foreach ($inscriptionConducteur->data['membres'] as $membreData) {
                ...
                if (isset($membreData['sacrements']) && is_array($membreData['sacrements'])) {
                    $this->createUserSacrements($userMembre, $membreData['sacrements']);
                } else {
                    $this->createUserSacrements($userMembre, null);
                }

                // 📧 Envoyer les identifiants et mot de passe par email au membre
                try {
                    if (!empty($membreData['email'])) {
                        Mail::send(new SendCredentials($userMembre, $identifierMembre, $tempPasswordMembre));
                    }
                } catch (\Exception $e) {
    ...
    private function createFamilyUsersFromInscription(Inscription $inscriptionResponsable)
    {
        // 1. Créer la famille
        $family = Family::create([
            'nom' => $inscriptionResponsable->data['famille']['nom'] ?? $inscriptionResponsable->nom . ' ' . $inscriptionResponsable->prenom,
            'classe_id' => $inscriptionResponsable->classe_id,
            'adresse' => $inscriptionResponsable->adresse,
            'quartier' => $inscriptionResponsable->data['famille']['quartier'] ?? null,
            'ville_id' => $inscriptionResponsable->ville_id,
            'telephone' => $inscriptionResponsable->telephone,
            'telephone2' => $inscriptionResponsable->telephone2,
            'email' => $inscriptionResponsable->email,
        ]);

        // 2. Créer l'utilisateur responsable
        $identifierResponsable = User::generateIdentifier(
            $inscriptionResponsable->nom,
            $inscriptionResponsable->prenom,
            $inscriptionResponsable->date_naissance?->format('Y-m-d')
        );

        $tempPasswordResponsable = '11111';

        $userResponsable = User::create([
            'nom' => $inscriptionResponsable->nom,
            'prenom' => $inscriptionResponsable->prenom,
            'email' => $inscriptionResponsable->email,
            'telephone' => $inscriptionResponsable->telephone,
            'telephone2' => $inscriptionResponsable->telephone2,
            'date_naissance' => $inscriptionResponsable->date_naissance,
            'genre' => $inscriptionResponsable->genre,
            'photo_path' => $inscriptionResponsable->photo_data,
        ...
        if (isset($inscriptionResponsable->data['sacrements']) && is_array($inscriptionResponsable->data['sacrements'])) {
            $this->createUserSacrements($userResponsable, $inscriptionResponsable->data['sacrements'], $inscriptionResponsable);
        } else {
            // Créer un record de sacrements même sans données initiales
            $this->createUserSacrements($userResponsable, null, $inscriptionResponsable);
        }
        ...
        try {
            Mail::send(new SendCredentials($userResponsable, $identifierResponsable, $tempPasswordResponsable));
        } catch (\Exception $e) {
