...
class TransferController extends Controller
{
    public function index()
    {
        ...
        $transfers = ClassTransferRequest::with([
            'family', 'user', 'sourceClass', 'targetClass',
            'validatedBySource', 'validatedByAccueil',
        ])
        ...
        try {
            ...
            if ($validated['type'] === 'member') {
                ...
                if ($member->is_family_responsible || ((int) $member->id === (int) $user->id && $user->is_family_responsible)) {
                    DB::rollBack();
                    return redirect()->back()->with('error', 'Le responsable de famille ne peut pas etre transfere seul.');
                }

                if ($transferService->isTransferLocked($member) || $transferService->hasPendingTransferForMember($member)) {
                    DB::rollBack();
                    return redirect()->back()->with('error', 'Ce membre est deja en cours de transfert.');
                }
            } else {
                if ($transferService->isTransferLocked($family) || $transferService->hasPendingTransferForFamily($family)) {
                    DB::rollBack();
                    return redirect()->back()->with('error', 'Cette famille est deja en cours de transfert.');
                }
            }
            ...
            $message = $isExternal
                ? "Demande de sortie externe créée (Ref: {$transfer->reference}). En attente de confirmation du conducteur."
                : "Demande de transfert créée (Ref: {$transfer->reference}). En attente de validation du conducteur source.";

            return redirect()
                ->route('bureau_conducteur.transferts.index')
