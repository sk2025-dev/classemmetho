...
class PresencesController extends Controller
{
    public function index()
    {
        if (! $this->hasRequiredTables()) {
            return Inertia::render('BureauConducteur/Presences/Index', [
                'stats' => [
                    'total_classes' => 0,
                    'total_membres' => 0,
                    'presents_dernier' => 0,
        ...
        return Inertia::render('BureauConducteur/Presences/Index', $this->collectDashboardData());
    }
    ...
    public function export(): StreamedResponse
    {
        ...
        $fileName = 'presences_bureau_conducteur_' . now()->format('Ymd_His') . '.csv';
    ...
    private function collectDashboardData(): array
    {
        ...
        $classes = Classe::query()->with('users:id,classe_id')->get(['id', 'nom']);
        ...
        $classActivitiesRows = Presence::query()
            ...
            ->map(function ($rows) use ($membresParClasse) {
                return collect($rows)->map(function ($row) use ($membresParClasse) {
        ...
        $participantsData = Presence::query()
            ...
            ->leftJoin('permanent_activities as pa', 'pa.id', '=', 'p.activite_id')
            ->leftJoin('special_events as se', 'se.id', '=', 'p.special_event_id')
            ->selectRaw('u.id as membre_id, u.prenom, u.nom, c.nom as classe_nom, p.statut,
                COALESCE(se.title, pa.title, "Activite") as activite_nom,
                p.marquee_le, p.created_at')
        ...
        if (Schema::hasTable('special_events')) {
            $events2026 = SpecialEvent::query()
                ...
                ->get([
                    'special_events.id', 'special_events.class_id', 'special_events.title',
                    'special_events.start_date', 'special_events.start_time', 'special_events.end_time',
                    'special_events.lieu', 'c.nom as classe_nom',
                ]);
        }
        ...
        foreach ($events2026 as $event) {
            ...
            if (! array_key_exists($monthKey, $activitesParMois2026)) continue;
