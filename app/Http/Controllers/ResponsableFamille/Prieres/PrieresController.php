...
class PrieresController extends Controller
{
        ...
        return Inertia::render('ResponsableFamille/Prieres/Index', [
            ...
            'receivedPrayerRequests' => $user
                ? PriereTargeting::filterVisibleForUser(
                    ...
                    ->map(fn (Priere $request) => PrierePresenter::forRecipient($request, 'responsable_famille'))
                    ->values()
                : [],
            'prayerRequests' => $user
                ? Priere::query()
    ...
    public function updateTestimony(Request $request, Priere $priere): RedirectResponse
    {
        ...
        $priere->addHistorique(
            ...
            'temoignage',
            $isOwner
                ? 'Un commentaire a ete ajoute par l auteur de la priere.'
                : 'Un commentaire a ete ajoute par le responsable destinataire.',
        );
        $priere->save();
    ...
    private function markPrayerAsInPrayer(Priere $priere, $user, string $status = 'En priere'): void
    {
        ...
        if ($priere->statut !== $status) {
            ...
            $priere->addHistorique(
                $user,
                'prise_en_priere',
                'La priere a ete prise en priere par un responsable de famille.',
                ['statut' => $status],
            );
            $priere->save();
        }
