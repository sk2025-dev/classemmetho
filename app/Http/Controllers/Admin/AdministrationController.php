...
class AdministrationController extends Controller
{
    public function index()
    {
        ...
        $allUsers = User::with(['family', 'family.responsable', 'classe', 'ville', 'fonction', 'sacrements'])->get();
        $totalUsersCount = $allUsers->count();

        // Classes (exclure les soft-deleted)
        $classes = Classe::withoutTrashed()->get();
        ...
        $classesFormatted = Classe::withoutTrashed()->get()->map(function ($c) use ($allUsers) {
            // Compter les utilisateurs de cette classe
            $usersInClasse = $allUsers->filter(function ($u) use ($c) {
                return $u->classe_id === $c->id;
            });

            // Compter les utilisateurs ACTIFS de cette classe (SANS les admins/conducteurs/fonctions)
            $usersActiveInClasse = $allUsers->filter(function ($u) use ($c) {
                $isInClass = $u->classe_id === $c->id && $u->is_active === true;
                $isNotAdmin = $u->role !== 'admin' && $u->role !== 'conducteur' && $u->role !== 'fonction';
            ...
            return [
                ...
                'membres_active_count' => $usersActiveInClasse->count(),
        ...
        $membersFormatted = $allUsers->map(function ($m) {
            ...
            return [
                ...
                'photo' => PhotoHelper::getPhotoUrl($m->photo_path, $m->prenom, $m->nom),
                'profil_photo' => PhotoHelper::getPhotoUrl($m->photo_path, $m->prenom, $m->nom),
                'role' => $m->role ?? 'utilisateur',
                'statut' => $m->statut,
                'is_active' => $m->statut === 'actif' ? 1 : 0,
