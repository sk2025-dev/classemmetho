...
class PresencesController extends Controller
{
    ...
    private function collectDashboardData(): array
    {
        ...
        // Taux de présence global (toutes classes, toutes activités)
        $totalPresences = (int) Presence::query()->count();
        $totalPresents  = (int) Presence::query()->where('statut', 'present')->count();
        $presentsDernier = $totalPresences > 0
            ? min(100, (int) round(($totalPresents / $totalPresences) * 100))
            : 0;
        ...
        // Nombre de membres (responsable + membre famille) par classe — source de vérité pour les %
                ...
                return collect($rows)
                    ->map(function ($row) use ($membresParClasse) {
                        ...
                        return [
                            ...
                            'pct'    => $pct,
                            'present' => $present,
                            'absent'  => max(0, $totalMembres - $present),
                            'total'   => $totalMembres,
        ...
        // Top activités du mois en cours (par nombre de présents)
        ...
        $topActivitesMois = Presence::query()
            ...
            ->map(function ($r) use ($membresParClasse) {
                ...
                $pct            = $totalMembres > 0 ? (int) round(($presents / $totalMembres) * 100) : 0;
