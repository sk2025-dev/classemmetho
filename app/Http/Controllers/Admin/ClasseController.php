...
class ClasseController extends Controller
{
    ...
    public function store(Request $request)
    {
        $validated = $request->validate([
            'nom' => [
                ...
                'string',
                'max:255',
                Rule::unique('classes', 'nom')->withoutTrashed(),
            ],
    ...
    {
        \Log::info('📝 UPDATE() appelé - ID from URL: ' . $id);

        // Chercher la classe manuellement
        $classe = Classe::findOrFail($id);
        \Log::info('✓ Classe trouvée: ID=' . $classe->id . ', Nom=' . $classe->nom);

        $validated = $request->validate([
            'nom' => ['required', 'string', 'max:255', Rule::unique('classes', 'nom')->ignore($classe->id)->whereNull('deleted_at')],
            'description' => 'nullable|string',
            'conducteur' => 'nullable|string|max:255',
            'status' => 'nullable|in:active,inactive',
        ]);

        \Log::info('✅ Validation réussie - Validated data: ' . json_encode($validated));
        ...
        \Log::info('💾 Classe mise à jour - ID: ' . $classe->id . ', Nouveau nom: ' . $classe->nom);
    ...
    {
        // Récupérer la classe AVEC les soft-deleted
        $classe = Classe::withTrashed()->findOrFail($id);

        \Log::info('🗑️ destroy() appelé - Classe ID: ' . $classe->id . ', Nom: ' . $classe->nom);
        ...
        try {
            ...
            // 3. Retourner JSON - le frontend fera le reload
            return response()->json([
                'success' => true,
                'message' => "Classe '$nomClasse' supprimée avec succès",
                'classe_id' => $classeId,
                'classe_nom' => $nomClasse
            ]);
