...
class PresenceController extends Controller
{
    public function store(Request $request): JsonResponse
    {
        ...
        if (now()->lt($programmeStartAt)) {
            $remainingMinutes = now()->diffInMinutes($programmeStartAt);
            $remainingLabel = $remainingMinutes >= 60
                ? sprintf('%dh%02d', intdiv($remainingMinutes, 60), $remainingMinutes % 60)
        ...
        $toInsert = $members
            ...
            ->map(function (User $member) use ($event) {
                return [
                    ...
                    'notes' => 'Absence automatique (non scanné avant fin de l\'activité)',
                    'created_at' => now(),
                    'updated_at' => now(),
                ];
            })
