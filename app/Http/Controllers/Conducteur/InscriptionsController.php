...
class InscriptionsController extends Controller
{
    ...
    {
        return app(TransferWorkflowService::class)->isTransferLocked($member);
    }
    ...
    public function index()
    {
        ...
        try {
            ...
            $members = User::whereIn('classe_id', $classIds)
                ->where('role', '!=', 'admin')
                ->with(['sacrements', 'fonctions', 'classe', 'ville', 'family:id,nom,code_famille,contact_urgence,contact_urgence_tel'])
                ->get();
            ...
            $members = $transferService
                ...
                ->map(function ($member) {
                    ...
                    $statutMarital = null;
                    $originFamilyCode = $this->getOriginFamilyCode($member);
                    ...
                    if ($member->sacrements) {
                        if ($member->sacrements->est_marie) {
                            ...
                            $statutMarital = 'Veuf(ve)';
                        } else {
                            $statutMarital = 'Célibataire';
                        }
                    } elseif ($member->statut_marital ?? null) {
                        $statutMarital = $member->statut_marital;
                    }
                    ...
                    return [
                        ...
                        'fonctions_eglise' => $member->fonctions->pluck('nom')->filter()->values()->all(),
                        'classe_nom' => $member->classe?->nom ?? null,
                        'ville_nom' => $member->ville?->nom ?? null,
                        'contact_urgence' => $member->family?->contact_urgence ?? null,
                        'contact_urgence_tel' => $member->family?->contact_urgence_tel ?? null,
            ...
            try {
                ...
                $families = $transferService->hydrateFamiliesTransferState($families);

                $directoryUsers = $families
                    ->flatMap(function ($family) {
                        $users = collect();

                        if ($family->responsable) {
                            $users->push($family->responsable);
                        }

                        return $users->concat($family->users);
                    })
                    ->unique('id')
                    ->values();

                $hydratedDirectoryUsers = $transferService
                    ->hydrateUsersTransferState($directoryUsers)
                    ->keyBy('id');

                $families = $families
                    ->map(function ($family) use ($hydratedDirectoryUsers) {
                        if ($family->responsable) {
                            $family->setRelation(
                                'responsable',
                                $hydratedDirectoryUsers->get($family->responsable->id, $family->responsable)
                            );
                        }

                        $family->setRelation(
                            'users',
                            $family->users->map(fn ($user) => $hydratedDirectoryUsers->get($user->id, $user))
                        );
                    ...
                    ->map(function ($family) {
                        ...
                        return [
                            ...
                            'responsable' => [
                                ...
                                'transfer_locked' => (bool) $family->responsable->transfer_locked,
                            ...
                            'members' => $family->users
                                ...
                                $originFamilyCode = $this->getOriginFamilyCode($user);
                                $familyCodeHistory = collect([
                                    $originFamilyCode,
                                    $family->code_famille,
                                ])->filter()->unique()->values()->all();
                                ...
                                return [
                                    ...
                                    'transfer_history_label' => count($familyCodeHistory) > 1 ? 'Nouveau membre' : null,
                                    'transfer_locked' => (bool) $user->transfer_locked,
                                    'is_deceased' => (bool) $user->is_deceased,
                                    'deceased_at' => $user->deceased_at?->format('Y-m-d'),
                                    'genre' => $user->genre ?? '',
            ...
            // Transferts en attente d'action de CE conducteur
            // 1) Il est conducteur SOURCE → doit approuver le départ (EN_ATTENTE_SOURCE)
            $pendingAsSource = \App\Models\ClassTransferRequest::whereIn('classe_source_id', $classIds)
                ->where('statut', 'EN_ATTENTE_SOURCE')
                ->count();

            // 2) Il est conducteur D'ACCUEIL → doit approuver l'arrivée (EN_ATTENTE_ACCUEIL)
            $pendingAsTarget = \App\Models\ClassTransferRequest::whereIn('classe_cible_id', $classIds)
                ->where('statut', 'EN_ATTENTE_ACCUEIL')
                ->count();

            $pendingTransfersCount = $pendingAsSource + $pendingAsTarget;

            $flashInfoAnnonces = ActeLiturgique::annonces()
                ->whereNull('family_id')
            ...
            return Inertia::render('Conducteur/Inscriptions', [
                ...
                'flashInfoAnnonces' => $flashInfoAnnonces,
            ]);
        } catch (\Exception $e) {
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            if ($familyId) {
                ...
                if ($family && app(TransferWorkflowService::class)->isTransferLocked($family)) {
    ...
    public function update(Request $request, $memberId)
    {
        ...
        try {
            ...
            if ($request->hasFile('photo')) {
                ...
                $member->photo_path = $request->file('photo')->store('photos/users', 'public');
                $member->save();
            }
    ...
    public function storeFamily(Request $request)
    {
        ...
        try {
            ...
            $validated = $request->validate([
                ...
                'famille.ville' => 'nullable|exists:villes,id',
    ...
    private function getOriginFamilyCode(User $user): ?string
    {
        return $user->getRelation('computedOriginFamilyTransferRequest')?->family?->code_famille;
    }
}
