...
class TransferWorkflowController extends Controller
{
    public function index()
    {
        ...
        $processedTransfers = ClassTransferRequest::with([
            ...
            ->whereIn('statut', ['TERMINEE', 'REFUSEE', 'EN_ATTENTE_PASTEUR'])
            ->where(function ($query) use ($user) {
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $transfer = ClassTransferRequest::create([
                ...
                // Externe → en attente du pasteur pour validation finale
                'status' => $isExternal ? 'EN_ATTENTE_PASTEUR' : 'EN_ATTENTE_ACCUEIL',
            ...
            // Pour les deux cas on verrouille le membre/famille pendant que c'est en attente
    ...
    public function approveAsSource($id)
    {
        ...
        if ($this->isExternalTransfer($transfer)) {
            try {
                ...
                // Le conducteur valide la source, mais c'est le pasteur qui clôture
                $transfer->update([
                    'status' => 'EN_ATTENTE_PASTEUR',
                    'validated_by_source_id' => $user->id,
                    'validated_by_source_at' => now(),
                ]);
                ...
                DB::commit();
                return redirect()->back()->with('success', 'Sortie externe transmise au pasteur pour validation finale.');
            } catch (\Exception $e) {
                DB::rollBack();
