...
class PresenceConducteurController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return Inertia::render(
            ...
            $this->sanitizePayload($this->buildPresencePagePayload($user, true, false)),
        );
    }
    ...
    public function indexMarqueur(Request $request)
    {
        ...
        return Inertia::render(
            ...
            $this->sanitizePayload($this->buildPresencePagePayload($user, false, true)),
        );
    }
        ...
        $items = SpecialEvent::query()
            ...
            ->map(function (SpecialEvent $event) {
                ...
                return [
                    ...
                    'source' => 'programme',
                    'lieu' => $this->utf8Safe($event->lieu),
                ];
            })
            ->values();

        return response()->json($this->sanitizePayload([
            'success' => true,
            'activites' => $items,
        ]));
    }
    ...
    public function assignPresenceMarker(Request $request): JsonResponse
    {
        ...
        return response()->json([
            ...
            'data' => [
                'id' => $member->id,
                'nom' => trim($this->utf8Safe($member->prenom) . ' ' . $this->utf8Safe($member->nom)),
                'famille' => $this->utf8Safe($member->family?->nom) ?: 'Sans famille',
            ],
        ]);
    }
    ...
    private function buildPresencePagePayload(User $user, bool $canManagePresenceMarker, bool $isMarkerView): array
    {
        ...
        return [
            'conducteur' => [
                'id' => $user->id,
                'prenom' => $this->utf8Safe($user->prenom),
                'nom' => $this->utf8Safe($user->nom),
                'classe' => ['id' => $classe->id, 'nom' => $this->utf8Safe($classe->nom)],
            ],
            'viewerLabel' => $isMarkerView ? 'Marqueur' : 'Conducteur',
            'canManagePresenceMarker' => $canManagePresenceMarker,
            'presenceMarkerClasse' => $marqueur ? [
                'id' => $marqueur->id,
                'nom' => trim($this->utf8Safe($marqueur->prenom) . ' ' . $this->utf8Safe($marqueur->nom)),
                'famille' => $this->utf8Safe($marqueur->family?->nom) ?: 'Sans famille',
            ] : null,
            'presenceEndpoints' => [
    ...
    private function utf8Safe(?string $str): string
    {
        ...
        $result = @iconv('ISO-8859-1', 'UTF-8//IGNORE', $str);
    ...
    {
        if (is_string($data)) {
            return $this->utf8Safe($data);
        }
        if ($data instanceof \Illuminate\Support\Collection) {
            return $data->map(fn($item) => $this->sanitizePayload($item))->values()->toArray();
        }
        if (is_array($data)) {
            $result = [];
