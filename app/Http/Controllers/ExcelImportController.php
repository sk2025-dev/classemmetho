...
class ExcelImportController extends Controller
{
    ...
    private function importUserRow($row, $lineNumber, $familyMap, &$result)
    {
        ...
        $fonctionIds = $this->getFonctionIds($row['fonction'] ?? null);
        ...
        $userData = [
            ...
            'fonction_id'           => $fonctionIds[0] ?? null,
            'relation'              => $this->normalizeRelation($row['relation'] ?? null),
    ...
    private function getVilleId($villeName): ?int
    {
        ...
        if (isset($this->villeCache[$key])) {
            return $this->villeCache[$key]->id;
        }
        $ville = \App\Models\Ville::firstOrCreate(['nom' => trim($villeName)]);
        $this->villeCache[$key] = $ville;
        return $ville->id;
    }
    ...
    private function normalizeRelation(?string $value): ?string
    {
        ...
        $map = [
            'pere'=>'Père','père'=>'Père','PERE'=>'Père','PÈRE'=>'Père',
            'mere'=>'Mère','mère'=>'Mère','MERE'=>'Mère','MÈRE'=>'Mère',
            'fils'=>'Fils','FILS'=>'Fils',
            'fille'=>'Fille','FILLE'=>'Fille',
            'frere'=>'Frère','frère'=>'Frère','FRERE'=>'Frère','FRÈRE'=>'Frère',
            'soeur'=>'Sœur','sœur'=>'Sœur','SOEUR'=>'Sœur','SŒUR'=>'Sœur',
            'oncle'=>'Oncle','ONCLE'=>'Oncle',
            'tante'=>'Tante','TANTE'=>'Tante',
            'cousin'=>'Cousin','COUSIN'=>'Cousin',
            'cousine'=>'Cousine','COUSINE'=>'Cousine',
            'epoux'=>'Époux','époux'=>'Époux','EPOUX'=>'Époux',
            'epouse'=>'Épouse','épouse'=>'Épouse','EPOUSE'=>'Épouse',
            'grandpere'=>'Grand-père','grand-père'=>'Grand-père','grand-pere'=>'Grand-père',
            'grandmere'=>'Grand-mère','grand-mère'=>'Grand-mère','grand-mere'=>'Grand-mère',
            'beaupere'=>'Beau-père','beau-père'=>'Beau-père','beau-pere'=>'Beau-père',
            'bellemere'=>'Belle-mère','belle-mère'=>'Belle-mère','belle-mere'=>'Belle-mère',
            'beaufils'=>'Beau-fils','beau-fils'=>'Beau-fils',
            'beaufille'=>'Belle-fille','belle-fille'=>'Belle-fille',
            'tuteur'=>'Tuteur','TUTEUR'=>'Tuteur',
            'tutrice'=>'Tutrice','TUTRICE'=>'Tutrice',
            'neveu'=>'Neveu','NEVEU'=>'Neveu',
            'niece'=>'Nièce','nièce'=>'Nièce','NIECE'=>'Nièce',
            'gendre'=>'Gendre','GENDRE'=>'Gendre',
            'conjoint'=>'Conjoint(e)','conjoint(e)'=>'Conjoint(e)',
            'parrain'=>'Parrain','PARRAIN'=>'Parrain',
            'marraine'=>'Marraine','MARRAINE'=>'Marraine',
    ...
    {
        return $this->getFonctionIds($fonctionName)[0] ?? null;
    }

    private function getFonctionIds($fonctionValue): array
    {
        if (empty($fonctionValue)) {
            return [];
        }

        $names = $this->splitFonctions((string) $fonctionValue);
        $ids = [];
        ...
        foreach ($names as $name) {
            ...
            if (isset($this->fonctionCache[$key])) {
                $ids[] = $this->fonctionCache[$key]->id;
                continue;
            }

            $fonction = \App\Models\Fonction::firstOrCreate(['nom' => trim($name)]);
    ...
    private function splitFonctions(string $value): array
    {
        $raw = trim($value);
        if ($raw === '') {
            return [];
        ...
        $parts = preg_split('/\s*(?:,|;|\||\/|\bet\b)\s*/iu', $raw) ?: [];
