...
class ExcelImportController extends Controller
{
    ...
    private function importUserRow($row, $lineNumber, $familyMap, &$result)
    {
        ...
        $email = $hasRealEmail
            ? $rawEmail
            : $this->generateFallbackImportEmail($firstName, $lastName, $dateNaissance, $familyId, $lineNumber);
    ...
    /**
     * Génère un email technique unique pour les imports sans email.
     * Domaine .invalid pour éviter tout envoi réel.
     */
    private function generateFallbackImportEmail($nom, $prenom, $dateNaissance, $familyId, $lineNumber): string
    {
        $safeNom = strtolower(preg_replace('/[^a-z0-9]/i', '', (string) $nom));
        $safePrenom = strtolower(preg_replace('/[^a-z0-9]/i', '', (string) $prenom));
        $datePart = $dateNaissance ? str_replace('-', '', (string) $dateNaissance) : now()->format('Ymd');
        $famPart = $familyId ?: 0;

        $base = "{$safeNom}.{$safePrenom}.{$datePart}.f{$famPart}.l{$lineNumber}";
        $email = "{$base}@import-local.invalid";
        $suffix = 1;

        while (User::where('email', $email)->exists()) {
            $email = "{$base}.{$suffix}@import-local.invalid";
            $suffix++;
        }
