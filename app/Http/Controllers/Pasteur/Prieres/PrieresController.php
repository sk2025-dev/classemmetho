...
namespace App\Http\Controllers\Pasteur\Prieres;
...
class PrieresController extends Controller
{
    ...
    public function index(): Response
    {
        ...
        return Inertia::render('Pasteur/Prieres/Index', [
            ...
            'prayerRequests' => PriereTargeting::filterVisibleForUser(
                ...
                ->map(fn (Priere $request) => PrierePresenter::forRecipient($request, 'pasteur', $user?->id))
                ->values(),
        ]);
    }
    ...
    public function addComment(Request $request, Priere $priere): RedirectResponse
    {
        ...
        $priere->addHistorique($user, 'commentaire_pasteur', 'Un commentaire a ete ajoute par le pasteur.');
        $priere->save();

        return back()->with('success', 'Le commentaire du pasteur a ete ajoute.');
    }
