...
namespace App\Http\Controllers\MembreFamille\Prieres;
...
class PrieresController extends Controller
{
    public function index(): Response
    {
        ...
        return Inertia::render('MembreFamille/Prieres/Index', [
            ...
            'targeting' => $user ? PriereTargeting::availableTargetsForMember($user) : null,
            'prayerRequests' => $user
                ? Priere::query()
                    ->with('requester')
            ...
            'receivedPrayerRequests' => $user
                ? PriereTargeting::filterVisibleForUser(
                    ...
                    ->map(fn (Priere $request) => PrierePresenter::forRecipient($request, 'membre_famille'))
                    ->values()
                : [],
        ]);
    }
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        $destinations = PriereTargeting::buildDestinationsForMember(
            $user,
    ...
    public function updateTestimony(Request $request, Priere $priere): RedirectResponse
    {
        ...
        $priere->addHistorique(
            ...
            'temoignage',
            $isOwner
                ? 'Un commentaire a ete ajoute par le membre.'
                : 'Un commentaire a ete ajoute par le destinataire de la priere.',
        );
        $priere->save();
    ...
    private function markPrayerAsInPrayer(Priere $priere, $user, string $status = 'En priere'): void
    {
        ...
        if ($priere->statut !== $status) {
            ...
            $priere->addHistorique(
                $user,
                'prise_en_priere',
                'La priere a ete prise en priere par un membre de famille.',
                ['statut' => $status],
            );
            $priere->save();
        }
