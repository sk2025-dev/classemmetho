...
class LiturgieController extends Controller
{
    ...
    {
        return Inertia::render('MembreFamille/Liturgie/Selection');
    }
    ...
    public function createForm(Request $request)
    {
        ...
        return Inertia::render('MembreFamille/Liturgie/Form', [
            'actes' => $actes,
            'familyMembers' => $familyMembers,
            'classes' => $classes,
    ...
    public function store(StoreActeLiturgiqueRequest $request)
    {
        ...
        if (!$isMemberInFamily) {
            return response()->json([
                'success' => false,
                'message' => 'Le membre sélectionné ne fait pas partie de votre famille.',
            ], 422);
        }
        ...
        if ($targetMemberId > 0 && !empty($payload['type_acte'])) {
            ...
            if ($already) {
                return response()->json([
                    'success' => false,
                    'message' => 'Vous avez déjà soumis une demande de ' . str_replace('_', ' ', $payload['type_acte']) . '.',
                ], 422);
            }
        }
        ...
        return response()->json([
            'success' => true,
            'message' => 'Demande liturgique soumise avec succès.',
    ...
    public function certificat(int $id)
    {
        ...
        if (!in_array($acte->statut, ['CELEBRE', 'TERMINE'], true)) {
            abort(422, "Le certificat est disponible uniquement après l'acte effectué.");
        }
    ...
    private function loadFamilyActesData(User $user): array
    {
        ...
        $actes = ActeLiturgique::with(['membre', 'classe', 'createur', 'historiques.acteur'])
